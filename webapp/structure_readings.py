...
def build_structure_readings_artifact(
    *, rows: Sequence[Mapping[str, Any]], parse_errors: Sequence[str] = (),
    ...
    payload: dict[str, Any] = {
        ...
        # `nodes` 是給人讀的標籤（插槽帶［socket］）；`items` 是給命令用的結構化 {node, unit}——
        # 拿標籤直接跑 `--check` 會找不到節點（plan 待決 #13，R2-b N2；Step 2.7）。
        "needs_reread": {"n": len(needs), "nodes": [_row_label(r) for r in needs],
                         "items": [{"node": r.get("node"), "unit": r.get("unit")} for r in needs],
        ...
        "disproof_triggers": {"n": len(triggers),
                              ...
                              "items": [{"node": r.get("node"), "unit": r.get("unit")} for r in triggers],
