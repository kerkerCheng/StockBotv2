...
**輕量版面（2026-09-02 使用者定案：實際投入頻率約半年一次；同日補充修正）：** 砍的是**段落與重複敘述，不是表格欄位**——Beta 區不再有 TL;DR 段（目標句收斂為一行）與「低於目標可優先補」複述行，portfolio risk threshold 只在實際跨越時出現。逐檔表欄位保留完整（心跳 1／5／20 日＋52 週區間位置、距高點、距 SMA200）：使用者原話「他是橫向滑的……我想要的話我就右滑再看就好」——表格欄位在橫向捲動下不構成版面成本，縱向的段落才是。心跳契約的最小要求一項不減。
