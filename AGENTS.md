...
3. **（已完成 2026-07-28）Daily Beta Technical Monitor v1** — plan：[`docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md`](docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md)。
   ...
   - **Engine D：** `config/beta_policy.json` 是 v1 numeric SSOT；signal只選 `0／0.25／0.5／1.0` pace。Sheet cash 仍先扣5% operating＋3% alpha reserve，多標的共用同一份 frozen deployable cash sequential allocation；campaign budget 控制單輪節奏。投組硬擋只剩 daily-reset ETF：內部 `nominal_weight` 是「投入槓桿 ETF 的資金占 NAV」5/8% warning/cap，`effective_weight` 才是乘上 2x／3x 後的「換算槓桿曝險」15/20% warning/cap；面向使用者不得再把前者寫成模糊的「名目槓桿」。investment policy 的單筆5%上限亦為硬擋；issuer concentration、alpha 總量與 drawn loan leverage 只 warning，不消耗 contribution capacity。
   - **操作與邊界：** `& '.venv\Scripts\python.exe' scripts\daily_beta_snapshot.py --format markdown --risk-view changes` 已加入06:30 Daily fixed entry與窄 automation allowlist。輸出明標 `policy_mode=paper_observation`、`capital_scope=sheet_conservative`；不建立choice／fill、不下單、不寫Sheet，也不把undrawn loan算資本。真實驗收讀到約11條series，DRAM短歷史降級，其餘可產signal；現況數值每日變動，以最新run為準。
   ...
   - **Beta mobile presentation contract（2026-07-30 使用者定案）：** 底層仍保存 `sheet_conservative_range` 與 `household_cash_supported_range` 兩條 fail-safe calculation，但首屏不得並列成兩種「可部署現金」。Capital Authority 可用時，使用它作唯一的「自有現金可部署」與「本輪可評估上限」；不可用時才顯示 `Sheet 保守備援`，並在健康段落說明降級。另獨立顯示「未動用貸款額度」，明標不算自有現金、未納入本輪上限。不得用未解釋的斜線或 raw field name。燈號固定配文字：🟢可評估、🟡冷卻／排序中、⚪觀察、🔴資料不足／暫停新增；Codex desktop 若支援 inline mobile visual，依「自有現金／本輪上限／貸款餘額 → 風險燈號 → 標的燈號」層級呈現，其他 executor 輸出等價 Markdown。
   ...
   - **（已完成 2026-07-28）Phase II-A Household Capital Authority：** plan：[`docs/plans/2026-07-28-002-feat-household-capital-authority-plan.md`](docs/plans/2026-07-28-002-feat-household-capital-authority-plan.md)。私人 Google Sheet `Capital Authority` 保存 household actual values；runtime credential scope 仍只有 `spreadsheets.readonly`，不建立／修改 tab 或 cell。自有可投資現金只認 `Portfolio` cash，私人 floor／planned outflows／未動用額度分欄；point-in-time view 保存 freshness、exact-direction FX 與 content digest。Runtime／JSON 仍並列 Phase I `sheet_conservative_range` 與 `household_cash_supported_range`，另分開 `contingent_credit_available`、`loan_funded_supported_range=manual_review_required`；authority／FX 失敗只歸零 household path，不抹掉 Phase I。Daily 人類首屏則依上方 presentation contract 只顯示一條自有現金主路徑，Sheet range 只在 Capital Authority 失效時作備援。未動用額度不算 NAV／cash／allocation，每次提款、標的與 tranche 仍是 explicit manual review，「高信心」不構成 machine permission。真實唯讀驗收可讀 Portfolio＋Capital Authority＋TWD/USD；私人 Sheet 未另行同步前仍可誠實顯示 `credit_terms_incomplete` telemetry。ETF完整look-through、live promotion、explicit-fill-only Sheet writer與server各自後移；zero-code-first LLM advisory仍只在使用者主動提問時按需執行。
...
4. **（已完成 2026-07-29）Portfolio Risk Policy Redesign** — plan：[`docs/plans/2026-07-29-001-refactor-portfolio-risk-policy-plan.md`](docs/plans/2026-07-29-001-refactor-portfolio-risk-policy-plan.md)。
   - **統一政策：** `config/investment_policy.json` v2026-07-29.2 與 `config/beta_policy.json` v2026-07-29.3 是 numeric SSOT；只有 ETF 槓桿 cap 與5%單筆上限會把 live supported range 歸零，其餘曝險只記錄／警告。使用者仍可走既有 prepared `live_override` 留下 exact action＋reason receipt；系統不自動下單。
