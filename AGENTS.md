...
2. **Daily Approval Loop（進行中，大型開發）**
   — 每日 harvest／triage／brief／封閉動詞核准迴路。可實作 plan：[`docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md`](docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md)。進度：**U1、U2 已完成**。U1＝`engine_b/leads.py` 狀態機＋去重、`crons/harvest_leads.py` RSS＋EDGAR harvest、seeded `library/leads/pending_leads.json`（21 tests；真實 harvest 驗證）。U2＝修好 `evaluate-signal` 對「有 ticker 但缺 execution metadata」公司在 context freeze 階段的 crash（改 `decision_lab/context.py`：core identity 決定 resolved、execution metadata 缺失只是 lane blocker），並補齊 6 檔美股 watch 的 registry metadata。**U3 已完成**＝MCP gateway 新增第 10 個工具 `get_decision_brief`（READ_ONLY，pass-through `decision_lab today` 的 redacted DTO；Decision Store 永不進 git，這是手機／雲端看決策佇列的唯一視窗；`mcp_server/decision_tools.py` core＋5 tests）。剩 U4（`/daily-brief` skill）、U5（daily cloud routine）。SIVE 找 3 個獨立 `origin_entity` 來源仍為獨立待辦。
