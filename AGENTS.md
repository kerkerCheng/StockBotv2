...
5. **同一套紀律適用於自己的「技術診斷」，不只引用的事實（2026-08-19 補）。** 一天內三個診斷落地後被推翻，共同形狀是**錯誤朝「有洞察力的結論」偏**，且每個都能用本檔的 lesson 語言包裝——**能套進某條 L 只代表值得查，不代表已經查過**。落地前先跑一條**試圖讓自己的結論變成假**的命令（不是驗證它為真）；三次都存在 30 秒內可否證的命令而沒人跑。已撤回的診斷與各自的否證命令累積在 [`ROADMAP.md`「已撤回的診斷」](docs/ROADMAP.md)，開工前掃一遍。
...
5. **同一個形狀也會出現在「找資料」上，而且那次是自己踩的（2026-08-15）。**
   查 IQE 客戶集中度時搜 `accounted for`（過去式），年報寫的是 `account for`（現在式），
   四輪查詢全回「未揭露」，於是推論出「這個 gate 對非美股結構性不可及、是牆不是閘門」——
   **一個時態差異造出一條架構級假結論**，且它剛好嵌得進當時正在寫的敘事（L11 確認偏誤）。
   真相是 SIVE 用年報 Note 5、IQE 用 Note 4.3，兩者都揭露。
   - **關鍵字未命中不等於資料不存在。** 下「未揭露」結論前必須改用語意定位（找分部附註／
     IFRS 8 段落／目錄），而不是換幾個關鍵字再放棄。
   - **工具回報的「沒有」要先問它是不是「讀不到」。** 同輪 WebFetch 對 8.6MB PDF 回
     `NO MAJOR CUSTOMER NOTE`，同時自陳解析失敗——兩義訊號（L12）。改用本機 `pypdf`
     解析並先驗證抽取品質（字元數、空白頁數）才有資格下結論。
   - 通則：**「我找不到」與「它不存在」是兩個不同的 claim，後者的舉證責任高得多。**
