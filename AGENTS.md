...
**輕量版面（2026-09-02 使用者定案：實際投入頻率約半年一次）：** 逐檔表只留四欄——行情狀態、心跳（最新完整交易日＋1 日漲跌）、52 週區間位置、sleeve 配置狀態；**5／20 日報酬、距 52 週高點、距 SMA200 不再逐檔列出**，Beta 區也不再有 TL;DR 段與「低於目標可優先補」複述行。這**不是**放寬心跳契約——上一段的最小要求一項不減，砍的是附加欄位與重複敘述；未來要把欄位加回去屬呈現層變更，需使用者再定案。
