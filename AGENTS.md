...
2. **Daily Approval Loop（進行中，大型開發）**
   — 每日 harvest／triage／brief／封閉動詞核准迴路。可實作 plan：[`docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md`](docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md)。進度：**U1、U2 已完成**。U1＝`engine_b/leads.py` 狀態機＋去重、`crons/harvest_leads.py` RSS＋EDGAR harvest、seeded `library/leads/pending_leads.json`（21 tests；真實 harvest 驗證）。U2＝修好 `evaluate-signal` 對「有 ticker 但缺 execution metadata」公司在 context freeze 階段的 crash（改 `decision_lab/context.py`：core identity 決定 resolved、execution metadata 缺失只是 lane blocker），並補齊 6 檔美股 watch 的 registry metadata。**U3、U4 已完成。** U3＝MCP gateway 第 10 個工具 `get_decision_brief`（READ_ONLY，遠端看決策佇列的唯一視窗）。U4＝`skills/daily-brief/SKILL.md`（四佇列 action-first brief＋封閉動詞 research／apply／park／skip）、`engine_b/cli.py`（leads list／triage／advance／counts）、`crons/pending_leads_digest.py`（SessionStart 提示待判斷數，雙通道）。剩 U5（daily cloud routine prompt）。
   - **每日操作：** 本機說「daily brief」或跑 `/daily-brief` skill → harvest → triage → 今日決策 → thesis 到期 → 一句動詞核准。Leads CLI：`python -m engine_b.cli list|triage|advance|counts`。decision_lab 命令只在本機（雲端 clone 無 private store）。SIVE 找 3 個獨立 `origin_entity` 來源仍為獨立待辦。
