...
3. **（已完成 2026-07-28）Daily Beta Technical Monitor v1** — plan：[`docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md`](docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md)。
   ...
   - **Phase II：** household capital authority、ETF point-in-time完整look-through、live policy promotion、explicit-fill-only Sheet narrow writer；只有本機漏跑不可接受、需要intraday／market-close即時通知或provider SLA時才升級最小server worker。另保留 zero-code-first 問答型 LLM advisory：使用者主動問黑天鵝／regime／替代策略時，按需補抓最新客觀資料後直接回答；不跑每日 LLM 研究、不建 Engine A 式 graph／新 authority、不自動進 pq，也不得放寬 deterministic hard caps。只有重複取數摩擦證明有價值時才考慮唯讀 compact context composer。Paywall ROI與通用Daily runner仍留相鄰backlog，不納入本切片。
