...
2. **（已完成 2026-07-23）Daily Approval Loop** — 每日 harvest／triage／brief／封閉動詞核准迴路。歷史 plan：[`docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md`](docs/plans/2026-07-22-002-feat-daily-approval-loop-plan.md)。U1 leads 狀態機＋harvest、U2 partial-identity crash 修復＋registry、U3 MCP `get_decision_brief`、U4 `/daily-brief` skill＋leads CLI＋session digest、U5 daily cloud routine prompt。**剩 rollout（人工）：** 使用者在 claude.ai 建 daily routine（`crons/daily_brief_prompt.md`，台北 06:30）→ 連續 3 天 bake。
