...
2. **（進行中，大型開發）Daily Approval Loop v1.1** — active plan：[`docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md`](docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md)。把 v1.0 骨架升級成：優先研究佇列（priority gate pq1 昂貴階段）、可續跑 drain（lead-status checkpoint）、閉環 evidence-delta（入圖→連結 probe→回 brief）、對話式批次核准（`1 3 go 4 drop`，**無 GitHub UI**）、MCP leads 窄 pathset commit/push 同步、Shadow-first 追蹤、lifecycle hook+手動、weekly 瘦身。U1 priority/drain → U2 MCP leads → U3 閉環 → U4 批次核准 UX → U5 routine/lifecycle/docs。
