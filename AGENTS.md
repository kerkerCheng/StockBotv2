...
2. **（已完成 2026-07-26）Daily Approval Loop v1.2 本機 rollout** — v1.1 plan：[`docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md`](docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md)。原 U1–U5 均保留；現行 runner 改成 Codex desktop local scheduled task（daily 台北 06:30；weekly 週日 04:00），直接讀 `.env`、Neo4j、Engine C／Decision Store，不再依賴 Claude cloud clone／MCP 才能完成 brief。新增 Engine C daily ETL、repo `.venv` 明確入口與窄 state publisher；兩個排程都在 master 執行且刻意錯開。
   ...
   - **Routine 分工：** daily（`crons/daily_brief_prompt.md`）＝X／EDGAR harvest＋Engine C financial／beta technical ETL＋Sheet-conservative／household-cash 並列 beta monitor＋triage＋today＋統一 pq2 brief；weekly（`crons/weekly_scan_prompt.md`）＝topic discovery＋完整本機健康審查＋唯讀 lifecycle。兩者都不替使用者寫 thesis 結論、入圖或 live facts。
...
3. **（已完成 2026-07-28）Daily Beta Technical Monitor v1** — plan：[`docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md`](docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md)。
   ...
   - **Engine D：** `config/beta_policy.json` 是 v1 numeric SSOT；signal只選 `0／0.25／0.5／1.0` pace，hard maximum另由Sheet cash扣5% operating＋3% alpha reserve、campaign budget、combined leverage 5/8% nominal warning/cap、15/20% effective warning/cap、tech proxy 60/70%與known issuer 30/35%控制。多標的共用同一份 frozen deployable cash sequential allocation，不得各自重複使用全額現金。
   - **操作與邊界：** `& '.venv\Scripts\python.exe' scripts\daily_beta_snapshot.py --format markdown` 已加入06:30 Daily fixed entry與窄 automation allowlist。輸出明標 `policy_mode=paper_observation`、`capital_scope=sheet_conservative`；不建立choice／fill、不下單、不寫Sheet，也不把undrawn loan算資本。真實驗收讀到約11條series，DRAM短歷史降級，其餘可產signal；現況數值每日變動，以最新run為準。
   ...
   - **Live portfolio 曝險的現金與 ETF 邊界（2026-07-29）：** Sheet 的 `bucket=CASH` 列由 adapter 標 `is_cash` 並傳入 frozen context，`decision_lab/sizing.py` 的 `_live_portfolio` 跳過它們——現金計入 NAV，但沒有 company／factor 曝險可解析，先前被誤報成 `holdings_company_mapping_unresolved:—` 而擋住 live sizing。**其餘 `holdings_company_mapping_unresolved:*`（ETF 與未入 registry 的權值股）與 `holdings_factor_mapping_unresolved:*` 是 ETF look-through 未做的正確 fail-closed 表現，不得靠補 registry 資料草率消音。** 實測佐證：以 beta policy `technology_proxy_load` 估算的科技曝險約占 NAV 55%，而 alpha factor 目前只算得出 photonics／small_cap 各約 1.5%（僅 Sivers 一檔）；只補 single-name factor_tags 而不做 ETF look-through，會讓 `factor_exposure_caps`（semiconductor 25%）建立在嚴重低估的分母上，比誠實 blocker 更危險。要清這批必須連同 ETF look-through 與 factor 分類體系一起做。
   - **（已完成 2026-07-28）Phase II-A Household Capital Authority：** plan：[`docs/plans/2026-07-28-002-feat-household-capital-authority-plan.md`](docs/plans/2026-07-28-002-feat-household-capital-authority-plan.md)。私人 Google Sheet `Capital Authority` 保存 household actual values；runtime credential scope 仍只有 `spreadsheets.readonly`，不建立／修改 tab 或 cell。自有可投資現金只認 `Portfolio` cash，私人 floor／planned outflows／未動用額度分欄；point-in-time view 保存 freshness、exact-direction FX 與 content digest。Daily 並列 Phase I `sheet_conservative_range` 與 `household_cash_supported_range`，另分開 `contingent_credit_available`、`loan_funded_supported_range=manual_review_required`；authority／FX 失敗只歸零 household path，不抹掉 Phase I。未動用額度不算 NAV／cash／allocation，每次提款、標的與 tranche 仍是 explicit manual review，「高信心」不構成 machine permission。真實唯讀驗收可讀 Portfolio＋Capital Authority＋TWD/USD；私人 Sheet 未另行同步前仍可誠實顯示 `credit_terms_incomplete` telemetry。ETF完整look-through、live promotion、explicit-fill-only Sheet writer與server各自後移；zero-code-first LLM advisory仍只在使用者主動提問時按需執行。
