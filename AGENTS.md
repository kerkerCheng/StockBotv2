...
2. **（已完成 2026-07-26）Daily Approval Loop v1.2 本機 rollout** — v1.1 plan：[`docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md`](docs/plans/2026-07-24-001-feat-daily-approval-loop-v1-1-plan.md)。原 U1–U5 均保留；現行 runner 改成 Codex desktop local scheduled task（daily 台北 06:30；weekly 週日 04:00），直接讀 `.env`、Neo4j、Engine C／Decision Store，不再依賴 Claude cloud clone／MCP 才能完成 brief。新增 Engine C daily ETL、repo `.venv` 明確入口與窄 state publisher；兩個排程都在 master 執行且刻意錯開。
   ...
   - **Routine 分工：** daily（`crons/daily_brief_prompt.md`）＝X／EDGAR harvest＋Engine C financial／beta technical ETL＋Sheet-conservative beta monitor＋triage＋today＋統一 pq2 brief；weekly（`crons/weekly_scan_prompt.md`）＝topic discovery＋完整本機健康審查＋唯讀 lifecycle。兩者都不替使用者寫 thesis 結論、入圖或 live facts。
...
3. **（已完成 2026-07-28）Daily Beta Technical Monitor v1** — plan：[`docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md`](docs/plans/2026-07-28-001-feat-daily-beta-technical-monitor-plan.md)。
   ...
   - **Phase II-A（brainstorm settled、尚未實作）：** 私人 Google Sheet `Capital Authority` 保存 household actual values；自有可投資現金只認 `Portfolio` cash 欄位，禁止重複加總，私人 operating floor／planned outflows／未動用房屋擔保額度分欄。使用者不預先禁止 loan proceeds 投入 broad beta、3x 或 alpha，但每次提款、標的與 tranche 都是 explicit manual review；Daily 必須並列 Phase I `sheet_conservative_range` 與新的 `household_cash_supported_range`，再分開 `contingent_credit_available`、`loan_funded_supported_range=manual_review_required`。私人 operating floor 只作用於 household candidate，既有 5% NAV operating＋3% alpha reserve range 不被靜默覆蓋；額度不得算 NAV，也不得因「高信心」自動放行。下一個開發起點只做唯讀 Capital Authority adapter＋point-in-time capital view＋Daily 四欄輸出；ETF完整look-through、live policy promotion、explicit-fill-only Sheet writer與server各自後移。zero-code-first LLM advisory維持使用者主動提問才按需抓最新資料，不進每日LLM研究／graph／pq、不改hard caps；只有重複取數摩擦才考慮compact context composer。Paywall ROI與通用Daily runner仍留相鄰backlog。
