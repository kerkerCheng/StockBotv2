...
⚠ **本段是補上 2026-08-15 契約缺的另一半。** 原契約把「不給尺寸」寫了三段，
「要給什麼」只有開頭半句且未定義判準，於是實際行為退化成**只列清單、不排序**——
2026-08-19 使用者要求推薦時，agent 以「outcome 0/8 未驗證」為由拒絕排序，
使用者的回應是「這像是我根本沒出手點，然後你就留了一堆反證給我，這樣要幹嘛」。

**使用者的實際用法（原話）：「哪個公司佔據了瓶頸、且是市場資金關注的部分，
那我們就去投，再來看 disproof。」** 排序判準依此固定為三維度：

1. **瓶頸地位** — `substitutability` 4–5／5、`sole_source`、`qualification_status`。
2. **資金關注** — **需求錨點是否存在、距需求端幾跳**。有 chokepoint 但 demand anchor
   為空者（實測如 GlobalFoundries 那批）不是候選，因為沒有資金在那條鏈上。
3. **證據強度** — 依 L8：`外部印證` ＞ `待判定` ＞ `供應商自報`。客戶端用錢印證
   （如 NVIDIA 對 COHR 的投資與產能協議）強度最高。

**唯一排序權威是 `query/bottleneck.py` 的 `rank_bottlenecks()`**，它已同時實作上述三者
（欄位：替代難度／證據／合格狀態／需求錨點／距需求端跳數）。不得另建平行排序，也不得
用 `axis_ceiling`、paper target 或 ELIGIBLE 數量代替它——那些是資本閘門，不是選股判準。

**交付要求（違反即視為未完成）：**
- 必須輸出**有序清單與明確的首選**，並直接回答「現在要加碼哪一檔」。
- 若因證據不足而無法排序，必須指出**缺哪一項具體證據**，不得以「未經 outcome 驗證」搪塞。
- **`outcome 0/8` 不是拒絕排序的理由。** 不出手就沒有 outcome，沒 outcome 就不敢排序，
  是死循環；L14 要求的是「不得讓未量測機制**決定資本尺寸**」，不是「不得表達研究判斷」。
  判斷與尺寸是兩件事，尺寸仍然不給。
