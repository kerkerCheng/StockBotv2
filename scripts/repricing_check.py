...
def _provider_close(ticker: str) -> tuple[str, float, bool] | None:
    ...
    抓取時戳也對得上：那筆的 `fetched_at` 是 **UTC 2026-08-17 22:33**（＝美東 08-17 18:33，
    收盤後約 2.5 小時；＝台北 08-18 06:33，daily routine 的排程時間）。所以
    `snapshot_date=08-18`（台北當地日）配 `bar_date=08-17`（美東交易日）**差一天是設計**，
    正是 2026-08-14 把兩者拆開的理由，不是誤差。
    ...
    ⚠ **那次誤判發生在盤中，這點值得單獨記著。** 對話時間是台北 08-18 22:00 ＝ 美東 08-18
    10:00，開盤約半小時。此時 `history()` 的最後一根是**進行中的 bar**，`Close` 欄放的是
    即時價——初版寫的「08-18 收 309.00」就是把盤中值當成收盤（實際收 306.43）。若當時
    provider 又暫時沒回 08-17 那根，看到的畫面確實會是「08-14 → 08-18」。

    所以錯的**不是觀察，是推論**：從「history 沒顯示 08-17」跳到「08-17 不是交易日」。
    這正是 L15 第 5 點——「我找不到」與「它不存在」是兩個不同的 claim，後者舉證責任高得多，
    而這裡它只需要一本日曆就能否證（08-17 是星期一）。留下的教訓不是「Engine C 的價格
    不可信」，而是 L11／L15：**在自己的診斷上要套用跟圖裡 claim 同一套追源紀律**；盤中
    尤其要先問「我現在看到的是收盤價還是進行中的 bar」。
    """
    try:
