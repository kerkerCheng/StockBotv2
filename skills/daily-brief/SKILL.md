...
| 標的 | 行情狀態 | 心跳（最新完整交易日＋1日） | 52週位置 | sleeve 狀態 |
|---|---|---|---|---|
| QQQ | 🟢 行情正常 | 2026-08-28：-0.6% | 85% | beta_tilt 高於目標 |
| TQQQ | 🟢 行情正常 | 2026-08-28：-2.0% | 69%（自身序列，未冒用 QQQ） | beta_leverage 到位 |
| 00631L.TW | 🔴 資料不足（Yahoo 落後 TWSE，暫時隔離） | Yahoo 2026-08-27：…；TWSE 08-28 +1.0% | 82%（降級，水位不可信） | beta_leverage 到位 |

**逐檔只留上述四欄（2026-09-02 使用者定案）：5／20 日報酬、距 52 週高點、距 SMA200
不再逐檔列出**——心跳契約要求的最小欄位是「最新完整交易日＋1日漲跌」，其餘是可砍的
附加欄。既有規則不變：quarantine 降級照舊現形（官方 reference 日期＋當日漲跌＋降級
原因）；52 週區間位置取自**自身**價格序列（TQQQ 不冒用 QQQ、00631L 不冒用 0050）；水位
只呈現、不參與排序、不換算金額，**不得用 RSI／MACD 等動能指標表達水位**（2026-08-01 實測
失敗的輸入，換名回歸即違規）；表末固定一行「長期上漲的標的多數時間落在高位是正確
資訊，不是該等回檔的訊號」（2026-07-31 回測：等回檔對 30 年終值是負貢獻）。
