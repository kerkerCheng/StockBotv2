...
⚠ **版面規定：決策區塊一律用 markdown 清單，不得用表格，也不得用 fenced code block。**
判準是 **markup 種類，不是字數**——決定手機上會不會換行的是渲染方式：表格與 code block
都是橫向捲動（`white-space: pre`），清單與段落才會自適應寬度。
**唯一該用 code fence 的是「要一鍵複製的東西」**（批次行、查證命令），而那些本來就短。

⚠ 2026-09-11 第一版改版時我把區塊放進 ```text fence——那等於換個方式製造同一個問題；
同版還寫了「每行 30 個全形字以內」，那是憑空的數字，不適應任何裝置。兩者都已撤銷。

每個欄位一個 bullet，**bullet 前面要有欄位標籤**——這樣換行之後仍然讀得懂是哪一欄：
...
- 修法層級：顯形（失敗仍會發生，但每輪會自己出現）——不做根除是因為「邊該不該有值」
  需要逐邊判斷，還沒有通解；**是加一個機制**（一個計數），拿不掉是因為今天沒有任何既有
  閘門在管這件事，沒有重複的東西可以移除；**錯了最先壞的是那 59 檔的計數本身**——去看過了，
  58 檔確實算不出結構分，只有 1 檔算得出
- 可逆：單一 commit，可 revert
- ROADMAP：無（T0 當下修不進 backlog——做完的東西不需要被排程）

⚠ **裸數字只屬於 pq2 編號空間，開發項不得用裸數字指稱**（2026-09-11 使用者當場抓到）。

開發項**不鑄號**（載體是 ROADMAP，`AGENTS.md` 2026-08-31 未動）。但 2026-09-11 我在同一則
訊息裡用 `9`、`11` 指 ROADMAP 的**列序號**，而同一則裡 `541`、`542` 是 pq2 編號——
兩者長得一模一樣，還收在同一條批次行上叫使用者回 `9 11 go`。使用者當場問
「所以現在開發項會鑄號？什麼時候變的」。**那不是誤會，是我把兩個命名空間混了。**

規則：
- **pq2 編號**寫成 `[N]`，N 是純數字。這是唯一能出現在批次行的編號。
- **ROADMAP 條目**一律用**標題引述**，不給序號：
  `ROADMAP「NEXT_PICK_RULE 沒有成本維度」`。序號會隨表格增刪而漂移，本來就不該當識別符。
- 兩者**不得出現在同一條批次行**。開發項要使用者點頭時，用「要不要做 X」的白話問句，
  不是一個可以被誤讀成編號的東西。

⚠ **「修法層級」欄對任何改動類項目都必填**（決策區塊與收據行都算），三檔是
**根除／顯形／個案**，判準與寫法見
[`skills/development-flow/SKILL.md`](../development-flow/SKILL.md) Step 3 第一條。
標「顯形」或「個案」時必須同欄寫出**為什麼不做根除**。研究類項目（go／drop 某個
研究動作）不填這一欄——它們不是修法。

⚠ **標題行、「為什麼現在」、「修法層級」三行不得出現識別符**（2026-09-13 使用者定案）。
識別符＝函式名、檔名、模組名、欄位名、封閉字彙的值、`co:*` ID，以及任何只有讀過原始碼才
還原得出主詞的內部代號。**它們只准出現在「會變的數字」與查證命令裡**——那兩格是給事後查
證用的，不是給當下決定用的。

**寫完自己跑一次這個檢查：把這三行單獨抄給一個沒讀過本 repo 的人，他答得出「這在解決什麼
問題、做完我會看到什麼不一樣」嗎？** 答不出來就**重寫那三行**，不是在後面補一句註解。

理由：使用者要核准的是**行為的改變**，不是實作方式。用實作名稱描述行為，等於要求他先讀
程式碼才能核准——而「不必讀程式碼也能決定」正是這個區塊存在的全部理由，也是
`AGENTS.md`「不得假設使用者能從 `co:*` ID 或內部術語自行還原主詞」那句話的同一條判準。
事發（2026-09-13）：一輪 research-drain 的三個待決項被寫成「`*_fx_tolerated` 只在報表幣別
≠ 共識幣別時才允許」「共識口徑核實加入第二條機械路徑（`interim_period_results`）」——
使用者的回應是「推薦做法是否根解」。**他必須先反問一輪才能開始判斷，那一輪就是這條規則的成本。**

⚠ **「會變的數字」與「可逆」兩格不得省略。** 前者是 L14（答不出哪個數字會變就不該做）；
後者是這類改動能走常規授權的**唯一理由**——不可逆的東西不在常規授權範圍內，要另外問。

### 一輪下來，哪一種改動怎麼出現在收尾（2026-09-11 使用者定案）

| 檔次 | 當場做？ | 收尾長什麼樣 | 進 ROADMAP？ |
|---|---|---|---|
| **T0 當下修**（十行內、不動 contract） | 做掉 | 收據行 | **不進**。做完的東西不需要被排程；寫進去只會長出下一批 ✅ 噪音 |
| **T1-a 單選題**（動 contract，但選項寫得出 A/B/C） | 見下方⚠ | 收據行，且必須寫出**被否決的替代方案**與理由 | 只有**原本就在** ROADMAP 的才回去標掉 |
...
卡住它的東西（intake 無更正路徑、unknown 軸卡死、NEXT_PICK_RULE 的兩條）。
**上一輪把下一輪的障礙全記下來了，因為沒有核准管道，下一輪又付了一次全額成本。**
所以 T1 一律當場問；ROADMAP 存在的理由是「使用者不在場時不遺失」，不是「先記著以後再說」。

⚠ **典型量級**（本 session 實測，供估算不是承諾）：一輪 research-drain 產出
**T0 約 2 個、T1 約 4–5 個、T2 0–1 個**。所以收尾的決策區塊通常是個位數，
不會讓使用者面對一張長清單——真的超過 8 個就代表這一輪混進了不該同批處理的東西。
