...
- **提案任何修法前，先答這四個問題，答案要寫進決策／收據區塊的「修法層級」欄**
  （①②2026-09-11 使用者定案；③④2026-09-13 使用者定案）：

  1. **這個修法讓失敗「不可能發生」，還是只讓它「被看見」？**
  2. **它靠執行者自律，還是靠程式？**
  3. **它是拿掉一個機制，還是加一個？** 加的要寫出**為什麼不能用拿掉的方式達成**。
  4. **如果它是錯的，最先壞掉的是哪一筆現有資料？** **去看那一筆**，把看到的結果寫進來。

  三檔：**根除**（失敗不可能再發生）＞ **顯形**（失敗仍會發生，但會自己出現）＞
  **個案**（只處理這一次這一例）。**標「顯形」或「個案」時必須寫出為什麼不做根除**
  ——成本、還不知道通解、或資料不足以泛化（L17-4：general 到資料支持的那一格為止）。
