...
這則材料的來源，看起來像不像跟這家公司現有來源不同的 `origin_entity`？**這是四要素裡對 L8 gate 進展最有價值的一項。**
...
**寬鬆原則的具體操作：** 四要素中，關聯性和可引用性是硬指標（沒關聯、沒有可查核內容 → 直接 FILTER，抽取了也沒用）。新穎性和潛在獨立性是軟指標，**任一項不確定或偏向「可能有價值」時，一律 PASS**，不要因為軟指標模糊就篩掉——這是「寧可多花一點抽取力氣、也不要悄悄漏掉好線索」的核心（R12）。
