# ROADMAP.md 逐字封存（2026-09-22 重建前：Alpha Edge Phase 0–7）

> 2026-09-22 依決定紀錄 G1–G11 重建 ROADMAP；本檔是重建前的逐字副本，**不再是現行路線圖**。
> 其中 amendment、backlog、已撤回的診斷、completion-gate 記錄全部在此。已撤回的診斷另抄一份到 `docs/lessons-incidents.md`。
...
## ⏸ 進行中的方向性重審（2026-09-21，**尚未有結論，不要據此行動**）

一次互動 session 對「估值層是否太重、排序能不能成立、圖怎麼消費」做了實測，
產出是**待審素材**，不是決定：
[`brainstorms/2026-09-21-graph-consumption-vs-ranking.md`](brainstorms/2026-09-21-graph-consumption-vs-ranking.md)。

**本檔一個 Phase 定義、驗收行、優先序都沒有因此改動**——七個診斷全部待否證，
作者同輪犯了兩次引用錯誤＋一次過度推廣（該檔第五節逐條記著）。
**下一步是獨立審查，不是實作。**

⚠ 讀下方〈還沒做的開發項〉那張表時要知道：**重審質疑的正是那張表的前提**
（它假設排序是對的、只是覆蓋不夠）。審查有結論前，那張表照舊有效，但別把它當成
唯一的待辦來源。
...
| **7** ⏸ | 問「要幾倍，哪一格得為真」，而不是「被低估幾 %」 | ~~多年橋**取代** FY+1 主流程；entry criterion 降級~~ **2026-09-20 修訂：寫下倍率射程的檔，首屏的「賭對了值多少」由多年視角回答；沒寫的維持 FY+1 並標明它是哪一種** | **機制端全做完**（倍率階梯／倍率射程／虧損公司改用營收倍數／進 APP／驗收腳本）。**差最後一步：首屏還沒接上多年視角**。<br>⏸ **2026-09-21 起暫停，等兩件事**：①下方〈②的一處自我更正〉的 (a)/(b)/(c) 仍未定案（2026-09-20 提出）；②方向性重審的獨立審查（見〈進行中的方向性重審〉）。**不是卡住，是刻意不推進**——首屏該接什麼，取決於審查後財務層還留多少地位 |
