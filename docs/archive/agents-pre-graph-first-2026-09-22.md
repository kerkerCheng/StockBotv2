# AGENTS.md 逐字封存（2026-09-22 換版前）

> 2026-09-22 依決定紀錄 G10／G11 換版；本檔是換版前 `AGENTS.md` 的逐字副本，**不再是現行規範**。
> 每段去向見 `docs/refactor/2026-09-22-agents-triage.md`。
...
**目標一句話（2026-09-16 使用者定案）：** 邊緣小公司的 **power-law 倍率**（2 到 10 倍，靠 AI capex 這類集中需求被放量的
瓶頸供應商），不是大型股的 20% 錯價，也不是 beta 波動。小賠多檔一檔補回；結構不變就抱；出場只認反證。決定紀錄：
[`docs/brainstorms/2026-09-16-alpha-edge-discovery-requirements.md`](docs/brainstorms/2026-09-16-alpha-edge-discovery-requirements.md)（D0–D15；整併後以本檔為準）。
...
**alpha 格自 2026-09-16 起只觀測、不設目標（D1）：** beta 五格目標保留（沒有目標時「誰跌深投誰」就回來）；
alpha 的比例只印不比，尺寸由使用者決定。config 落地隨 ROADMAP Phase 交付。
查證：`python -c "import json;print(json.load(open('config/target_allocation.json'))['sleeves']['alpha'])"`
——落地後應看到 `observed_only`，還看到 `target` 就是尚未落地。
...
**部位真相是 Google Sheet（2026-09-16 D14）：** Sheet 記貸款額度、投入標的與現金；Decision Store 只留可選的
receipt；下單由使用者透過 session 記進 Sheet（`scripts/record_trade.py`）。**alpha 原則上不用貸款資金**——這是
使用者自己的紀律，**系統不建 gate**（A5 不擴張）。
...
> **2026-09-16 整章重寫（使用者定案 D0）。** 舊章逐字封存於
> [`docs/archive/agents-presentation-contract-pre-alpha-edge.md`](docs/archive/agents-presentation-contract-pre-alpha-edge.md)，
> 每句去向見 [`docs/refactor/alpha-edge-step0-migration.md`](docs/refactor/alpha-edge-step0-migration.md)。
> 被取代的句子劃線加日期留原地，**不靜默刪除**；事發經過與當時數字一律指 archive。

## Alpha 呈現契約：power-law 漏斗——候選、賭注、反證與量測，不是部位尺寸

**系統只負責使用者自己做不動的事：哪些邊緣公司值得看、有什麼新事件、賭對了值多少、判斷錯了值多少、
哪個管道與特徵產出贏家。** 買多少、什麼時候買由使用者決定（D2）。**產出若無法讓人分辨做了什麼與沒做，它就不算產出。**

~~系統終點是**瓶頸度排序**，注意力狀態只剩 `MONITOR`／`REVIEW`。~~（2026-09-15 D1 改為籃子層排序＋個股頁賭注；
**2026-09-16 起終點是四層漏斗**。）
~~**outcome 量測改為等權重報酬追蹤**：只記「哪天推薦了這檔、當時股價、之後報酬率」。~~（2026-09-16 D15：主統計量換成
三個 power-law 統計量，等權中位數降為次要；仍不含部位大小或 NAV 佔比。）

資本表達層（`live_supported_range`／`axis_ceiling`／`paper_target`／probe cap／四動作）已於 2026-08-28 整組移除，理由與
實測值見 archive。**真正的風控完全不變**：拿掉的是憑空的建議尺寸，不是煞車；live choice／fill 仍 100% 人工，不連 broker；
**NAV 比例呈現是純呈現、零門檻**。查證（別相信這段文字，跑一次）：
`python -c "import json;p=json.load(open('config/investment_policy.json'));print(sorted(p['probe_lane']), p['single_position_nav_cap'])"`
——`probe_lane` 不該再有 `axis_ceilings` 等尺寸 cap；`single_position_nav_cap` 應仍是 0.05。

### 目標與儀器（2026-09-16 使用者定案）

- **目標是倍率不是錯價。** 2 到 10 倍的邊緣小公司；**小賠多檔一檔補回；結構不變就抱；出場只認反證**；
  目標價到了**只提醒**——`realized` 降為提醒，不觸發出場（D3）。
- **現行主流程（FY+1 EPS × 目標倍數找 20% 錯價）是給高覆蓋大型股的儀器**，對不上目標（事發：COHR 首屏「賭對了值 244、
  現價 266」，thesis 講 FY28，模型只有 FY27 一格）。**取代物是多年反向橋（「五倍要什麼為真」）**，排在 ROADMAP 最後一個
  Phase；落地前隱含報酬照印、照兩欄拆解，但**不得拿 FY+1 的負數當「不值得看」的結論**。
- **候選門檻是覆蓋厚薄，不是上市地（D11）：** `analyst_count`、市值；非英語 filing 是加分不是門檻。

### 四層漏斗（架構骨架；解法住 ROADMAP 各 Phase）

| 層 | 回答 | 呈現義務 |
|---|---|---|
| **發現** | 誰值得進佇列 | 來源標籤跟著 lead 走到 outcome。X 帳號登記表是**封閉清單**（tier `probation`／`measured`／`trusted`）＋每則貼文自動蓋章＋每週計分表五欄（D5）。**推文永遠是 tier-4 lead**；tier 只影響 pq1 優先序；升降是一季一次的 pq2 manual |
| **篩選** | 它是不是倍率候選 | **filter 不是分數**；INV-3 逐檔報 input／accepted／filtered／reasons；**籃子空就空，不得為了非空放寬條件**——讓它非空的路是研究（補三格：可替代性、外部印證、瓶頸業務占營收比例，D8） |
| **表達** | 怎麼買、怎麼抱、怎麼砍 | 賭注（variant overlay）與**判斷錯了值多少**（反證觸發後的假設套同一條橋）**對稱**；歸零旗標（現金跑道／負債／稀釋／going concern，紅黃綠不給數字）；**alpha 全歸零淨值少幾 %**（D2）。仍沒有 bull／bear、沒有機率加權 |
| **量測** | 哪個管道與特徵產出贏家 | 追蹤表三個 power-law 統計量（**12／24 個月內達 2 倍的比例、最大單檔貢獻、籃子總報酬**；D15）；計分表**必印量測起始日與樣本數**，並印三個已知偏差 |
...
- 必須輸出**有序清單與明確的首選**，直接回答「現在要加碼哪一檔」。**首選＝filter 不是分數**（2026-09-15 D2）：在排序
  權威的順序上套可機械驗證的條件，INV-3 逐檔報 reasons，沒有一檔通過就沒有首選、**不得放寬**。條件自 2026-09-16 起換成
  漏斗六條（見 ROADMAP Phase 4），落地前沿用三條（有賭注、payoff 為正、催化劑落在目標價日期前）。
- 無法排序就指出**缺哪一項具體證據**。**「outcome 還沒驗證」不是拒絕排序的理由**——不出手就沒有 outcome；L14 禁的是
  未量測機制**決定資本尺寸**，不是表達研究判斷。⚠ 刻意**不寫死比值**。
- 排序是**研究判斷**，必須明標它不是回測或統計勝率，並附各候選的 disproof。
- 必須點明**相關性**：本圖標的高度集中於 AI 光互連，N 檔不等於 N 個獨立機會；**主題翻轉時十檔一起 −50% 是使用者已接受
  的集中**——心跳只印回撤，不給燈號、不給動作。

**進場靠判斷，出場靠 disproof。** 反證用來決定何時認錯，不是進場的前置條件；混用會產生「永遠不出手、只累積反證」。

### 隱含報酬的兩個桿：EPS 與倍數適用同一條原則（2026-09-09 使用者定案）

**沒有 re-rating 證據時，目標倍數預設等於校準用的市場倍數**；折溢價必須指得出證據並寫進 rationale——與「沒有
differentiated evidence 時 EPS 收斂到 consensus 是健康的」是同一條原則。**隱含報酬必須拆成 EPS 差異貢獻、倍數差異貢獻兩欄。**
判準一句話：**一個保守選擇若說不出證據，它是偏差，不是審慎。**

### ⚠ 已知會失焦的指標——不得單獨用作瓶頸性證據

`evidence` 等級、同一 chokepoint 的供應商計數、`documents` 計數都是**研究量的函數**。
**判別法：這個指標會隨我們多讀一份文件而單調上升嗎？** 會 → 它測的是研究量。
2026-09-16 起多一條：**帳號計分表在樣本不足時量到的是倖存者**——所以 tier 升降只走一季一次的 pq2，且必須同時對 QQQ 與 SOXX 算超額。

## Beta 呈現契約：只回答「距目標多遠」與「現在在什麼水位」——開發凍結（2026-09-16 D6）

**beta 凍結開發，只保留「大盤比例」觀測**；個股呈現若擋路可拆，不為維持 beta 架構繞路。下列判準**一條沒改**：

- **`band` 是容忍區間不是 gate**——區間內即到位、沒有偏好。**再平衡只用新投入的錢往低於目標的格子補，不賣出**；
  只給差距，**不給金額、不排名、不產生部位尺寸**。貸款 tranche 不適用配置建議。
- **相對水位只呈現、不參與排序、不換算金額**，且必須寫明「長期上漲的標的多數時間落在高位是正確資訊，不是該等回檔的
  訊號」（2026-07-31 回測）。**一旦有人拿它排序或調整尺寸，它就變回訊號。**
- **燈號只表達行情資料狀態，不表達投入建議。** 舊語意（可評估／冷卻／暫停新增）已於 2026-08-29 明文**廢止**
  ——安靜消失擋不住下次回填，所以廢止必須寫出來。
- **逐檔行情表與目標配置表住 APP**（2026-09-08），Daily 只印門檻跨越與狀態翻轉；「即使沒有配置缺口逐檔表仍不得省略」
  已於 2026-09-08 廢止。**不變的 invariant：逐檔心跳必須永遠看得到，每列明示商品自身的最新完整交易日**；
  APP 當天沒被 materialize 時，Daily 必須把這件事印出來（L12）。
- **兩條相關性警告由 APP 常駐呈現**（頁尾），一字不改：（a）**alpha 與 beta 是同一個賭注**；（b）**TSMC look-through
  約 28%**，高於 `issuer_concentration_warning` 0.25，且系統算不出精確值（`issuer_loads` 覆蓋恆為 `partial`）。

呈現細節（欄位、燈號文字、台股 freshness、槓桿商品序列）見 [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) §8。

## APP 呈現契約：**LLM changes cognition; APP reads cognition**（2026-09-07 Step 5 定案）

**APP 讀的是已經形成的判讀。** request path **不得**：跑 LLM、寫任何 authority、抓外部資料、跑任何金融模型、重算隱含
報酬、或因為 cache miss／stale 就偷偷重建；要更新判讀只有明確跑一次 materialize——否則「使用者剛才看到的是哪一版判斷」
再也答不出來。查證：`python -m pytest tests/test_webapp_request_path.py`。

**首屏的單位是「句」不是「格」（2026-09-15 使用者定案）。** 第一屏只有投資人短評（七格前因後果＋一把尺＋一顆燈）；
第二層是**論證**（可長文）；**第三層才是格**（稽核區）。**消費層的單位是句與段；格只住稽核層。**
三條不可退讓：**文字由研究 session 寫進 append-only ledger（`alpha/narrative`）、數字由 authority 填 placeholder、
首屏禁字表型別層拒收。** 沒寫短評印「還沒寫短評」，不拿 thesis 硬截。查證：`python -m alpha brief COHR --list`。
2026-09-16 起那把尺多一端：**判斷錯了值多少**（D2）；歸零旗標是燈不是數字。

**缺席不得被壓成一句「無資料」。** `absence_kind` 由**產生缺席的那段程式自己宣告**，呈現層不得 parse 理由句去猜（L16）；
使用者必須分得出**還沒做／刻意不主張／方法不適用／上游缺料**。⚠ **「刻意不主張」只能來自 append-only 的 `Abstention`
紀錄**（必附 `reason` 與 `revisit_when`，結構上不可能帶數字）；**settled 不讓 readiness 變好**。

**`accounting_basis` 的標籤不得宣稱 authority 沒有的東西**：只分「as reported vs 公司調整後」，字彙不改（L10），
label 一律不含準則名稱。

**Daily 與 APP 的分工——昨天和今天一樣的住 APP，今天變了的＋要你決定的住 Daily**（2026-09-08）。pq2 的**核准**留在
Daily（載體是對話，APP 沒有寫入端點）；排序、配置、缺口、事件監看與「球在誰手上」四段清單（唯讀）住 APP。
⚠ **順序是 invariant：APP 先讀得到，Daily 才能不印**；**還沒有 APP 畫面的段落一律留在 Daily**，判準是機械的：
`python -m webapp status` 列得出那個 kind 才算搬完。

**Daily 拆三層（2026-09-16 D12；ROADMAP Phase 2 交付前是規格不是現況）：** **心跳**（零 LLM，固定五段）＋**分類**
（便宜模型、每日硬上限、失敗不阻斷）＋**研究**（只在互動 session；daily 的 `drain_limit_per_run` 歸零）。
硬規則：**LLM 失敗心跳照發；「未 triage N」必印**（L13）。五段規格與查證命令住 [`ARCHITECTURE.md`](docs/ARCHITECTURE.md) §4.1。

**APP 沒有任何寫入端點，也沒有自己的帳號密碼系統。** 認證邊界是 **Cloudflare Access**；預設只綁 `127.0.0.1`，綁其他
介面必須明示 `STOCKBOT_APP_ALLOW_PUBLIC_BIND=1`。**四個人工 gate 不因為多了一個畫面而放寬。**
...
**beta 定投擇時這條路已關，不是降級使用（commit `6aa31de`）。** RSI／MACD／`sma_50_slope`／tier／pace／
`campaign_budget_fraction_by_sleeve`／三態系統動作／「本輪可評估上限」在 **beta sleeve** 全部拔除，**不得以任何名義回到
beta 的文件或輸出**——包括改名成「熱度」「節奏」或借「水位」之名。
查證：`python -c "import json;print(sorted(json.load(open('config/beta_policy.json'))))"` 不應出現 `signal`。

**其餘 scope 適用通則，不是封殺（2026-09-10 使用者定案）：** 未經量測的指標**不得參與任何排序或尺寸決定**（INV-5／L14），
但**可以被提出、被測**。四個條件缺一不可：①**宣告 scenario**，不得跨 scenario 借用結論（兩個方向都不行）；②**先量測，
後放閘**；③過 L14-4 的三個免 outcome 測試（恆亮？不會滅？講不出因果機制？）；④驗收寫成「**現有資料有幾筆真的變了**」。
**D5 的帳號計分表是這條通則的第一個實例**：它是量測，tier 只動 pq1 優先序，**不參與排序、不給尺寸**。

⚠ **兩個數字不要混用：** 等權報酬追蹤（`library/private/decision_lab/outcome_aggregate.json`，時序在同目錄 `.jsonl`）
回答「追蹤了幾檔、跑了多久」；`measured_outcomes`（`library/private/app/state/positions.json` 的 `counters`）回答
「正式結算過幾筆」。2026-09-16 起前者的主統計量換成三個 power-law 統計量（D15）。樣本太少時驗證只會更弱——
outcome 追蹤是前置條件。

⚠ **機制性警告：「買跌最深的」會系統性把錢導向長期較弱的資產**（EFA CAGR 7.2%，被選了 98 次）。在 alpha 上更危險：
本圖標的同漲同跌，「哪檔跌最深」很可能只是「哪檔 beta 最高」。

**須區分量測、訊號與脈絡：** 總曝險倍數、歸零門檻、追繳門檻、利息覆蓋、**歸零旗標**屬**量測**；RSI／MACD／tier／pace
屬**訊號**（beta 定投擇時三次未過；其他 scenario 是未測，不是已否決）；位置指標是**呈現用的脈絡**——
