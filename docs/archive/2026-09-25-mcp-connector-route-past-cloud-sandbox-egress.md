# Solution：自建 connector 繞過 cloud sandbox egress 逐字封存（2026-09-25 退役）
...
> 本檔是退役前 `docs/solutions/architecture-patterns/mcp-connector-route-past-cloud-sandbox-egress.md` 的逐字副本（以下分隔線之後），**不再是現行規範、不得照著執行**。
