# 遠端存取架構 逐字封存（2026-09-25 退役）

> Phase 2 Step 2.1（ROADMAP 旁支「Graph MCP 退役」、plan 2026-09-25-001 §0.4 A6）：遠端 graph server、tunnel 的 `mcp.`／`neo4j.`
> hostname 與 claude.ai connector 全部移除；手機改用 Claude Code Remote Control 連本機 session，所有人工 gate 照舊。
> 本檔是退役前 `docs/remote-access-architecture.md` 的逐字副本（以下分隔線之後），**不再是現行規範、不得照著執行**。

---

# 遠端存取架構 — 資料流與安全邊界

> **⚠ 2026-09-24 起 Graph MCP 停用**（使用者決定）：`graph_mcp` process 已停、開機 vbs 已移除啟動行；`mcp.`、`neo4j.` 兩個 hostname 已移除（外部實測回 404）；claude.ai connector 已斷開。手機改用 Claude Code Remote Control 操作本機 session。
...
8. **遠端工具沒有 Git 能力**——`record_lead_decision` 只寫本機 leads authority；Engine B state 由 private backup 保護。圖 provenance 的 finalize／commit／push 仍只在明確的本機互動流程，MCP 不持有也不觸發 Git credential
