...
>
> ### 還有一件小的：Phase 2 Step 2.3（分類層的每日硬上限）
>
> ⚠⚠ **先更正一個 2026-09-19 傍晚寫錯的描述，免得再被它誤導一次。**
> 當時說 2.3 的價值之一是「不依賴 Codex」——**那是錯的**。
> `crons/daily_brief_prompt.md` 檔頭的三層分工表逐字把「**分類（triage）**」歸給
> 「**本檔（Codex 排程）**」，D12 也從來沒有要求分類層脫離 Codex。
> 「**失敗不阻斷**」的意思是「**分類層失敗不得阻斷心跳**」，而那**早就結構上成立了**
> ——心跳是獨立的 Windows 排程，根本不經 Codex。
>
> **所以 2.3 真正剩下的只有一件事：給 triage 一個每日硬上限**（`skills/signal-triage` 今天
> 由 Codex daily 跑，**沒有任何 cap**）。它**不是新增花費，是加一道煞車**——
> 先前把它說成「每天自動花錢」也是同一個誤解的延伸。
>
> **建議直接做，並建議上限設 30 則／天**：近 16 天實測每日新 lead **2–26 則**（平均約 15），
> 所以 30 是**保險絲不是配額**——平時不 binding，暴量那天才作用（先例：`harvest_config` 的
> `x_accounts.monthly_spend_cap_usd = 10`）。⚠ 觸發時**必須印「今天因上限跳過 N 則」**，
> 不得靜默（INV-3）；⚠ 依 L14-4，交付時要順便回答「它會不會恆亮／會不會永遠不滅」。
> 唯一要使用者點頭的是那個數字。
>
