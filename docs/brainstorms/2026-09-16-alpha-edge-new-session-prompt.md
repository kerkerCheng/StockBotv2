...
>
> **[595] 已 go 並結案**（2026-09-17）：兩筆讀圖已寫入，心跳第 2 段由「一份都還沒寫」→
> **「結構讀圖 2 份｜現行 2｜該重讀 0」**。兩筆各有一個值得記住的結果：
> ①`tech:cw_dfb_laser` 判 **volume**——供給側分布實測是 **5／3／3／2／2／2**（不是提案時以為的「全 2–3」）；
> Coherent 那個 5 的 evidence 是 `self_reported`，依 L8 不足以支撐 A 型的「明顯高於其他」。
> ⚠ 但真正更卡的在下一層：它 `depends_on mat:inp_substrate` 是 5。
> ②`mat:inp_substrate` 判 **undecided**——需求側 15 條裡 7 條 sub=5、2 條 sub=4、下一層 0 條（它是最底層），
> **整張圖說所有人都繞不過它，卻沒有人查過供它的三家彼此可不可替代**（AXT／住友電工／JX 全部沒填）。
> 那個缺口已鑄成 **[596]**。
>
> **等你決定的：** pq2 **[596]** 補 InP 基板供給側三條邊的 substitutability。
>
