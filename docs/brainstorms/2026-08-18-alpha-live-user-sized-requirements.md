...
這是**條件檢查，不是訊號**——不需要先證明任何預測能力，因此不受 D7 限制。

✅ **已實作（2026-08-18）：** `decision_lab/catalyst_watch.py` ＋
`scripts/catalyst_watch.py`，已接進 `skills/daily-brief` Step 4（否則它就是下一個
沒有入口的機制）。四態：`設定不完整`／`已逾期`／`即將到期`／`監控中`，
**`設定不完整` 排最前**——它的到期提醒本身就是假的。

資料源刻意是 Engine D 的 `coverage_assessments`（涵蓋全部 10 個 cohort），
不是 `thesis/lifecycle.json`（只有 3 條）。兩套 lifecycle 互不知道是既有的整合縫隙
（審查 C10、ROADMAP 已登記），這裡**不再蓋第三套**。

**散文日期不猜**（L15：語意可交給語言處理，狀態必須 deterministic）。只用結構化的
`expiry` 與 `catalyst_checkpoints`。報表末尾常駐顯示「N/M 檔有結構化催化劑日期」——
其餘檔的「expiry 早於催化劑」錯誤**測不到**，沒抓到問題不等於沒有問題（L13）。

**首跑即抓到三件事**（2026-08-18，10 檔中）：

| 標的 | 問題 |
|---|---|
| AVGO | catalyst 與 disproof 都是空字串——`expiry 2026-08-20` 是一個沒有內容的鬧鐘（pq2 [172] 待確認） |
| COHR | `expiry 2026-11-30` 早於它自己的催化劑 `2026-12-01`（推估）——差一天，但催化劑不可能在有效期內發生 |
| IQE.L | `expiry 2026-08-18` 今天逾期，而催化劑在 9 月 |

⚠ 目前只有 **3/10 檔**有結構化催化劑日期，所以「expiry 早於催化劑」這類錯誤在另外
7 檔身上測不到。補法是在 `thesis/lifecycle.json` 加 `catalyst_checkpoints`。
