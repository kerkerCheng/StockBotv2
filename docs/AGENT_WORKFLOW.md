...
使用者已常規授權：**Verdict 為 `GO`、且下一步沒有任何待他決定的問題時，agent 可直接接續下一個 Step，`Step` 與 `Phase` 邊界一視同仁**。
授權本文與**六條停止條件只住 `AGENTS.md`「協作與邊界」**（它是使用者給的授權，家在 AGENTS；本檔與 development-flow 都只指向，不抄）。
判準一句話：**這條授權買的是「不必為了說一聲而停」，不是「不必為了決定而停」。** 它不放寬任何 gate，也不改變 `GO` 的語意。

### 先給人看的 `HUMAN SUMMARY`（在 `STEP_RESULT` 之前）

`STEP_RESULT` 是**證據**：它讓使用者判斷「這個結果可不可信」。但 human owner 還要另一個問題的
答案——**「這一步對我的目標做了什麼、現在到哪、接下來要幹嘛」**。那不是八欄的工作，也不該靠
使用者自己從 artifact 名稱、內部編號與測試數字反推。

所以每個 Step 收尾**先**輸出一段 `HUMAN SUMMARY`（預設 5–10 行），**再**輸出八欄：

| 這一行回答 | 寫法 |
|---|---|
| **做了什麼** | 產品／責任層語言；**不先出現 module／class／artifact 名稱** |
| **為什麼重要** | 它讓使用者離 **Phase goal** 更近在哪裡 |
| **現在在哪** | 一行 progress map：✅ 已完成 → ▶ 這一步 → ○ 剩餘主要 Step |
| **下一步** | **What／Why now／After this** 三段，缺一不可 |

⚠ **它不是八欄的摘要。** 摘要只是把同樣的內容變短；HUMAN SUMMARY 回答的是另一個問題。
**若某一行只要把八欄剪貼過來就能寫出，那一行就沒有存在價值。**

⚠ **roadmap／內部 ID 首次出現必須附 plain-language title**（`B2b — 把研究缺口與事件監看搬成
APP 可持久讀取的狀態`）。使用者不必記得自己的編號系統——編號是給流程用的，不是給他用的。

⚠ **工程證據留在八欄。** 測試數、bytes、函式名不進 HUMAN SUMMARY，除非那個數字本身會改變
產品判斷。這條的反面同樣成立：**八欄不因為多了一段人話就可以少寫任何證據。**
