...
|---|---|---|---|---|
| **`AGENTS.md`** | **「我可以／不可以做什麼？」** | **會約束行為的規則**：invariant、authority 邊界、四個人工 gate、lesson 的判準句 | **每個 session 完整載入** | 771 → **≤450**（⚠ 2026-09-04 交付時改記字元數，見下方 §17.4） |
...
- ~~`AGENTS.md` **771 → ≤450 行**~~ → **改記字元數：52,229 → 26,147（−50%）**，且 INVARIANT **一條未刪**
  （2026-09-04 使用者裁決。差距全部來自折行慣例——舊檔 87 字元/非空行、新檔 49；同內容照舊密度
  重排約 300 行。450 行的**目的**是省 session context，而 context 成本是字元／token 不是行數；
  為湊行數重排成長行只會讓 diff 顆粒度變粗。**不是做不到就改分：內容確實砍掉一半。**）
