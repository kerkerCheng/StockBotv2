...
| 「等你決定」與「等事件」分離 | `config/decision_blockers.json` 的 `resolution_mode`（`user_decision`／`awaiting_external`／`system_internal`）；使用者可用 `pending --until/--trigger` 指定等待條件，優先於自動推導。G7 之後所有等待住 Event Watch registry（`library/leads/event_watches.json`）；「語意條件」kind（`semantic_condition`）2026-09-24 Phase 1 Step 1.4 交付：thesis／讀圖的反證條件原文逐字、喚醒目標 `disproof_ref`；T0 只比對 lead 的來源宣告（`source_class=primary`、非持股申報、實體交集、時間在建立之後），**不看 triage**；醒來＝待檢，daily 預篩只標旗，判定在互動 session（`event_watch semantic-queue` → `judge`） |
