...
| 財務核驗清單五項 | `engine_c/checklist.py` 的 `items` | L9 前置條件 #3。`gate_pass` 對全體取 `all()`，加第六項會讓**所有既有標的**的 gate 退化。⚠ 舊稱「Watchlist 升格 gate」已停用（該層 2026-09-02 除役），五項本身未廢止 |
| 營運假設的 driver | `alpha/fundamental/contracts.py` 的 `ASSUMPTION_DRIVERS`（`revenue_growth`／`operating_margin_delta`／`interest_and_other_net`／`tax_rate`／`nci_attribution`／`diluted_shares`） | **contract 不是 taxonomy**：每個 driver 對應 FY+1 因果橋的一段算術（⚠ 橋已於 2026-09-23 Phase 0 退役；driver 今天是 append-only 假設紀錄的身分與 `DisproofCondition.invalidates` 的封閉字彙），多一個 driver 就要多一段算術＋測試；自由字串會讓「寫得進 ledger 卻不影響任何數字」的假設出現（訊號二）。單位跟著 driver 走，寫入端驗 |
| 假設的知識種類 | 同檔 `ASSUMPTION_BASES`（`observation`／`heuristic_proxy`／`session_judgment`） | 是 read model `Basis` 的**子集**（`tests/test_alpha_view_fundamental.py` 斷言），刻意沒有 `deterministic`——輸入假設永遠不是確定性事實，確定性的是橋的算術 |
| 會計口徑 | 同檔 `ACCOUNTING_BASES`／`CONSENSUS_BASES`（⚠ `COMPARISON_STATUSES` 已於 2026-09-23 隨內部 vs 共識的數值比較退役） | `unverified` 口徑不得與任何口徑相減。多一種「不可比」要先想清楚 renderer 怎麼呈現它 |
...
| 估值方法 `VALUATION_METHODS`（＋`METHOD_PARAMETERS`／`METHOD_FUNDAMENTAL_INPUT`／`FAIR_VALUE_FORMULA`／`METHOD_ACCOUNTING_BASES`）與 horizon／entry 判準 | ~~`alpha/valuation/contracts.py`~~／~~`alpha/valuation/model.py`~~（整個 `alpha/valuation`、`alpha/implied_return` 套件與 `alpha/providers/{valuation_assumptions,horizon_assumptions,valuation_drift}.py`） | 2026-09-23 Phase 0 Step 0b.1b（C／H 組） | 估值鏈整條退役（ROADMAP Phase 0／G3：財務只回答三個是非題，**不得長回估值模型**）。方法字彙沒有新家——「已定價嗎」的主參照是自己的歷史，不設倍數、不設門檻。三本 private ledger 檔案留在 `library/private/alpha/{valuation,horizon,entry_criteria}/`（L10），但沒有讀寫入口 |
| FY+1 因果橋：`MODEL_VERSION`／`BRIDGE_VERSION`／`COMPARISON_STATUSES`／`OPINION_STANCES`／`opinion_stance` | ~~`alpha/fundamental/bridge.py`~~／~~`alpha/fundamental/model.py`~~（`contracts.py`／`assumptions.py`／`compare.py` **留**：資料契約、ledger 邏輯、口徑核實）；測試 ~~`tests/test_fundamental_model.py`~~ 退役，資料夾具搬到 `tests/fixtures_fundamental.py` | 2026-09-23 Phase 0 Step 0b.1b（C／H 組） | 內部 FY+1 預測是估值鏈的第一節。`ASSUMPTION_DRIVERS`／`ASSUMPTION_BASES`／`derivation`／`OPINION_BEARING_DRIVERS` **留**——它們是 append-only 假設紀錄的身分與寫入驗證字彙，不因模型退役而改（L10） |
