...
| 1.2b | 至少一次排程觸發成功後，**刪除**舊兩個 Windows 工作與 `crons/heartbeat_task.py`（不擋 1.3 起的 Step） | ✅ 前提：2026-09-25 05:30 第一次排程觸發 `Last Result 0`、run `4fe5bce1` 25 步齊全（ok 23／failed 2：⑦c＝§15 #14、⑮＝[586] 當時未補、現 13 PASS）；`--retire-legacy` 刪舊兩個工作，`schtasks /Query` 只剩 `StockBotv2-Daily`；舊檔刪除，第四條理由（LLM 失敗心跳照發）搬進 `daily_task.py`；5 條舊測試改主詞搬到 `test_daily_task.py`（4 個變異全紅）；全套 2 紅＝既有時區 flake，當下修測試（偏差 #28、§15 #16） | （本 commit） |
...
### R2-b 第三輪 findings（2026-09-24，對 `259147c`；VERDICT **GO**）

RB-1 以真實 registry 副本重跑（四種壞檔）：ew_0096～0111 一筆都沒被收、lifecycle 不算健康來源、修好後重新列出；RB-2 不再長出重複 watch
或兩個編號；設計 B 主路徑：等待不消失、同一條件不兩筆、每個到期事件恰一個重問的地方。依 L17（十行內、不動 contract 的當下修）處置如下：

| # | 問題 | 處置 |
|---|---|---|
| NB3-1 | 複查後續盯兩個洞：lifecycle 讀不到時照標 handled 卻不續盯；續盯到期早於條件原文日期被拒 → 條件掉進未盯 | **修**：讀不到就整筆不處理（不標 handled，下一筆複查再列）；續盯到期＝`rearm_until`（`review_horizon` 與條件日期＋30 取大）。測試＋忠實變異紅 |
| NB3-2 | 單純換位置時 `_reconcile_disproof` 不存檔，relink 只在記憶體 | **修**：存檔條件加 `relinked`；補用正式包裝的測試（fixture 替身曾蓋掉這個 bug） |
| NB3-3 | 準時複查時條件會在隔天各自到期、另開一筆（續盯＝核查點＋一個週期、到期用嚴格 `<`） | **修**：排程到期的那一筆一併帶上「一個核查週期內會到期」的條件（`upcoming_disproof_by_thesis`，理由「複查時一併續盯 N 條反證」），複查時 `ew.extend` 只延不縮。試跑：AXT 6 條在 2026-11-15 的排程複查就一併續盯 |
| NB3-4 | 重讀沒帶或帶錯 `supersedes_id` → 舊讀圖的到期條件永遠掛著；讀圖來源沒帶 node 時重讀理由列不到 | **修**：新讀圖以節點收舊（ledger 裡這個節點的其他讀圖 id）；讀圖來源的語意 watch 必帶 node |
| NB3-5 | (a) 使用者延後的複查被單純到期拉回、記成「反證被判觸及」；(b) `pending --until` 的日期過了沒有任何程式叫回（既有缺口，B 放大） | **修**：(a) 只有觸及（`disproof_touched_ids`）叫回，到期只附掛；(b) `_wake_passed_until`：日期過了就叫回、log `waiting_until_passed`（冪等） |
| NB3-6 | report 收據可用 `library/private/app/state/*.json`（含所有 watch_id）繞過 | **修**：只收 `docs/reports/`、`thesis/` 下的 `.md`；OPERATIONS 同步。報告日期檢查未做（go 只把假設型交回 fired，不涉 authority） |
| NB3-7 | A7 後的殘句：APP 標籤、`mark_expired` docstring、todo 註解、queue_segments、`GO_AUTHORIZATION["thesis_lifecycle"]`（L16：沒寫 go／drop 會續盯）；plan §8、§11 `Expiry`、§12 ④ | **修**：全數改寫；**§11 `Expiry` 依 A7 改寫**（thesis／讀圖來源檢查是否列進複查／重讀，只有假設型查 `watch_decision`），另加 `waiting_on.until` 已過的防呆 |
| NB3-8 | lifecycle 讀不到時 thesis 觸及被印成「孤兒」；memo 讀不到／解析不到時條件靜默從預期消失；sync 把對帳錯誤印成「登記失敗」 | **修**：讀不到時 thesis 那一半不算孤兒；計數多 `memo_unreadable`、心跳印出；sync 改印「對帳錯誤」 |
| NB3-9 | `register-disproof` 不去重（心跳還叫人去用它） | **修**：同一來源、同一條件已有處理中的等待就拒收（換 `#n` 也算同一條件） |
| NB3-10 | judge 之後、sync 之前換掉 memo，觸及待處置沒人接得住（A5 就有的邊角） | **記錄**：現在至少以「孤兒觸及 N」計數現形；處置入口列 closeout §15 |
| NB3-11 | 試跑所見：(a) drop 一筆仍到期的 thesis 複查，下次 sync 以同一標題重鑄（lifecycle 既有行為）；(b) 到期待複查的條件不再比對新文件（A3 起就是）；(c) 等重讀長期掛著只靠「該重讀」提醒；(d) 到期總數分項加不起來；(e) 複查項目附上條件時標題就地換掉、沒有「新增」標記 | (d)(e) 併 1.8（快照 diff、完整處置分布）；(a)(b)(c) **列 closeout §15 交使用者決定**——(b) 尤其是設計選擇：到期待複查期間要不要繼續比對 |
...
| 10 | 1.7 | 續等命令 `python -m engine_b.todo pending <n> --until <日期>` | `python -m engine_b.todo resolve <n> --verb pending --until <日期>`（hint、錯誤訊息、1.8 的心跳行都用這條） | `todo` 沒有 `pending` 子命令——照原文印出的是一條跑不動的命令（L11-6：先跑一次） |
| 11 | 1.7 | ref_id「帶 watch_id 與到期時間」 | ref_id＝`<watch_id>@<expires>`；收集器對「active 但 `expires` 已過」也照日期認；`watch_decision` 的動作先 `mark_expired` | CLI 的 sync 先收集、後由 `check_watches` 標到期：用 `expired_at` 的話到期當天沒有編號，隔天標記後換 key 會重鑄一號（變異檢查實測紅 3 條）。續等會改 `expires`，所以「續等後再到期＝新事件」仍成立 |
| 12 | 1.7 | （plan 未寫）到期待決的 thesis 條件、memo 已被取代或 thesis 已 retired | `todo sync` 的反證對帳對這種 expired 記 `expiry_resolution: source_superseded`，不鑄 `watch_decision` | 否則會問使用者一個已沒有對象的條件要不要續等；同一條對帳本來就收 active／fired 的同類 watch，這裡只是對稱面（L17） |
| 13 | 1.7 | `go` 的 receipt「指得回研究結果（lead id、報告路徑或新 watch id）」 | 只接受 `lead:`／`report:`／`watch:` 三種鍵、至少一項，**每一項都查得到**（lead 在 store、report 是 repo 內存在的檔、watch 在 registry）；追源型到期的結案也放進 ⑨ `consume-fired`（先 `mark_expired` 再結案），不等隔天 | 只檢查格式的話 `lead:隨便寫` 也過，「指得回」就只是字面；⑨ 在 ⑩ 之前跑，不先標到期會晚一天 |
| 14 | 1.6 | （plan 未寫）`disproof_items` 只取以 `- `／`1. ` 開頭的那一行 | 縮排續行併入前一條（中日文直接接、其餘補空格；空行結束） | AXT §7 第 2 條跨兩行，只取第一行會把條件截在「→ 「防禦性鎖客」」（L17：只認得當初那個案例的機制當下就修）；三份 memo 條數不變（6／5／5） |
...
| 20 | 1.8 | `engine_b/theme_scan.py`（`last_scan()`）與 `config/daily_routine.json` 的 `theme_scan.nudge_after_days` 列在 1.9 | 1.8 就建（心跳段 3「距上次掃題材 N 天」要用）；1.9 的 hook 直接共用 | 1.8 的那一行先要有資料源；拆兩次改同一個模組不划算 |
| 21 | 1.8 | 段 2 的 watch 行「今日醒／今日到期／標旗／未檢」取代舊的「事件監看：本輪該查／已觸發未消費／到期」 | 「本輪該查」（T2 主動輪詢的配額）不再印；「已觸發未消化」併進新 watch 行；到期改成段 3 的到期行（今日／累計＋處置逐格，併 NB2-8、NB3-11(d)） | 輪詢在 1.2a 已移出 daily，那個數沒有 consumer（INV-4）；NB3-11(e)「複查項目附上條件時標題就地換掉、沒有新增標記」**沒做**，列 closeout §15 |
| 22 | 1.8 | 新點名雷達「今天第一次被點名、**圖裡沒有**的名字」 | 以 **registry 沒有**當「圖裡沒有」的代理（圖裡的公司都在 registry）；沿用 `leads.onboard_candidates`（已 triage 的 lead、cashtag／手記名字），補 `first_seen` | 心跳不碰 Neo4j（零網路、只讀本機）；registry 另有 8 家尚未入圖，這個代理只會少報不會多報 |
| 23 | 1.8 | 快照「每次 daily 寫一份」 | 只有帶 `--write-snapshot` 才寫（daily ⑱ 帶；互動手跑不帶），並寫 Discord 摘要行檔 `heartbeat_<日期>.summary.txt`（⑲ 讀它當 `--summary`） | 互動手跑會蓋掉 daily 的那一份，讓隔天的「較昨」比到半天前的值 |
| 24 | 1.9 | hook 達門檻時輸出「systemMessage＋additionalContext」 | 只用 additionalContext 一個通道：達門檻（或從沒掃過）時內文以「【請在第一則回覆開頭轉述】」開頭，未達門檻是一行普通計數；與 daily 沒跑完的訊息合成同一段 | `crons/routine_hint.py` 既有定案：兩個通道同時輸出會讓 Codex desktop 顯示一次、agent 又轉述一次（同 `thesis_freshness_check.py` 的理由） |
| 25 | 1.9 | 「`rg -i weekly` 剩下的每一處都有理由」 | 留下的：描述退役經過的歷史句（OPERATIONS、ARCHITECTURE、CONCEPTS、docstring、config `_doc`）；舊報告檔名 `weekly_scan_<日期>.md` 的解析（`engine_b/theme_scan.py` 與其測試）；舊 lead 的 `weekly:` 來源標籤（`engine_b/leads.py`、`engine_b/cli.py` 註記、`tests/test_semantic_watch.py`）；`todo.py` 讀舊「Weekly topic：」項目的 legacy 分支與其測試；`docs/refactor/` 歷史文件；`mcp_server/`、`docs/remote-access-architecture.md` 的 MCP 相關（C5，旁支項拆除）；`crons/heartbeat_task.py`（1.2b 刪）；測試裡斷言「已退役／已拿掉」的那幾行 | 計畫列的「確定要留」全數照留；其餘改名或改寫成退役說明 |
...
| 收集器語意 | （⚠ **A7 之後只剩假設型等 `expiry_class=decision` 的**；thesis／讀圖來源的反證見 A7——列進 thesis 複查與節點重讀，不鑄號）語意型／假設型、`status=expired` 且沒有 `expiry_resolution` → 一列 `watch_decision`。**每個到期事件恰好一個編號**（ref_id＝`<watch_id>@<expires>`，不重鑄、不漏；偏差 #11）。標題不得只寫 `co:*` 或 watch_id（AGENTS：不得假設使用者能還原主詞）：「反證 watch 到期、條件沒發生：<條件前 40 字>（來源 <memo 或讀圖>）」 |
| 三個動詞 | `go`：receipt 必須指得回研究結果（lead id、報告路徑或新 watch id；`_validate_go_receipt` 加這一型），watch 記 `expiry_resolution`。**因為 go 要附研究結果，批次語法裡 bare `n go` 對這一型一定被拒**；批次語法也帶不了 `--until`（`engine_b/batch.py:19-36` 只認「編號＋動詞」），所以 bare `n pending` 同樣被拒——**心跳的批次行對這一型只列 `drop`**，另起一行給續等的完整命令 `python -m engine_b.todo resolve <n> --verb pending --until <日期>`（偏差 #10：`todo` 沒有 `pending` 子命令），並寫「要研究就在互動 session 說」（第 2 輪 N-h）。`drop`：watch 記 `expiry_resolution`（drop、編號、理由）。**續等（`pending --until <日期>` 在這一型上的 type-aware 語意）：watch 以新到期日回 `active`（歷史附加，不覆寫），該編號結案（`resolution="renewed"`）並在 receipt 記新到期日**——**等待只住 registry，不得同時掛在 pq2 的 `waiting_on`**（AGENTS：所有等待住同一個 registry；不建立第二個狀態源）。**這一型的 bare `pending`（沒帶 `--until`）拒收**，錯誤訊息說明要帶日期——否則它會照其他型別的語意變成「不結案、留在球在你」，同一個動詞兩種結果（L12：一個表示承載兩種語意）。`resolution` 多一個值 `renewed`：先 grep 所有假設 `resolution ∈ {go, drop}` 的讀取端（audit `Lifecycle`、todo list、心跳）一併處理。⚠ `_mark_source_cleared` 永不自動 resolve，所以結案要在同一個動作裡明確做 |
...
- **怎麼驗：** 測試：到期語意型 → 一個 `watch_decision`；再 sync 不重複；續等 → watch active、新到期、編號結案；drop → `expiry_resolution`；pq2 型到期 → 編號可動作；追源型 → 終局＋計數；
  ...
  `standing-go` 跳過 `watch_decision`；`tests/test_engine_b_todo.py` 兩個 dict 鍵一致；`tests/test_standing_authorization.py` 封閉性。**（A7 之後）假設型的 `watch_decision` 真實資料最早 2027-01-01；讀圖來源的反證最早 2026-11-18 到期、列進重讀理由**——照 A3。
...
- **怎麼改：**
  ...
  - `Expiry`（**依 A7 改寫**，R2-b 第三輪 NB3-7：原句照字面做會把 watch_decision「修」回 thesis／讀圖來源）：到期未處置、`expired_at` 超過一天的——`expiry_class=thesis_review` 若沒有被列進任何一筆未結案 `thesis_lifecycle` 的 `disproof_watch_ids`（lifecycle 讀得到時）→ **FAIL**；`reread` 若該節點的 `reread_reasons` 沒有它 → **FAIL**；`decision`（假設型等）若既沒有未結案的 `watch_decision`、也沒有 `expiry_resolution` → **FAIL**（INV-2：到期是重問不是丟）；pq2 型到期但指向的編號仍在 `waiting_on` → FAIL；`waiting_on.until` 已過卻仍掛著 → FAIL（NB3-5b 的防呆）。
...
10. **AXT `### 7b` 的 variant 推翻條件「Q3 2026 營收 ≥ US$60M」要不要登記**：不在「推翻」節，所以 1.6 沒登記（`docs/reports/2026-09-24-phase1-step16-disproof-registration.md` §1）。第 1 題的 39 條 claim 反證（AXT 11、COHR 12、Sivers 16）見同報告 §5。
11. **一則 Sivers 一手公告會同時叫醒 7 條語意 watch（thesis 5＋讀圖 2），預篩每日名額是 10**（同報告 §6）——工作量，不是錯。
12. **R2-b 第三輪 NB3-10（觸及待處置遇 memo 換版的處置入口）與 NB3-11 (a)(b)(c)(e) 試跑所見**：見本 plan「R2-b 第三輪 findings」表。
13. **`pending --trigger` 不帶 `--until` 仍被接受**（偏差 #27）：`trigger` 沒有任何程式讀，等待住在 registry 外、永不重問。提案：`--trigger` 必須同時帶 `--until` 或綁一個 watch（動 CLI contract）。當下唯一一筆 [586] 已由使用者 2026-09-25 補 `--until 2026-12-31`。
14. **追源 watch 把「trace_status 已終局」的舊 lead 排回 pq1，排回後缺分類**（2026-09-25 第一次自然觸發實例：一則新的 AXTI PASS 叫醒 `ew_0091`／`ew_0093`，兩則 7 月的 AXTI 8-K〔`lead_f6d7fdd6…`、`lead_44ce140c…`，`trace_status=not_pursued`〕以 triaged_go 回到 pq1；它們第一次 triage 時還沒有分類欄位，`requeue_trace` 沒得沿用 → daily ⑦c `classification-health` exit 2、drain 列 withheld）。燈是設計上要亮的（缺漏顯形、不偷 slot）；補救路徑存在（互動 session：park → pending → 帶分類重新 triage，要讀那份 8-K，屬 pq1 研究）。機制題：已終局的 trace 要不要被排回、排回時缺分類要怎麼接——使用者決定。同型的 watch 另有十餘筆（parked＋終局 trace_status），查證：`python -m engine_b.cli trace-backlog --needs-attention`。
15. **使用者動作**：要用 Codex 互動 session 的話，需信任 1.9 新增的 SessionStart hook（Codex 側未驗）。
16. **watch 的「今天」是 UTC 日期，daily 在台北 05:30（＝UTC 前一天 21:30）跑**（1.2b 全套測試在台北 07:15 撞出來，偏差 #28）：
    `engine_b/event_watch.py::_today()` 取 `datetime.now(timezone.utc).date()`，到期（`expires < today`）、`--until` 須晚於今天、
    登記的 `watch_id` 日期都用它——所以 daily 裡的到期判斷**一律比本地日期晚一天**（12-31 到期的 watch 要到 01-02 那一輪才轉 expired，
