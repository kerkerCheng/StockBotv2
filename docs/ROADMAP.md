...
Step 3 Entry Logic ✅（2026-09-06）
  ...
  → Coverage Pilot（2–3 檔 heterogeneous）   ← 下一站
  → Step 5 APP／API
  → Post-MVP Alpha Edge（A–F）
  → Portfolio／Risk later
```

**Coverage Pilot 的意義**：到 Step 4 為止整條鏈只在 COHR 一檔上跑過，
所有「fail closed」都是在**同一組資料形狀**上驗的。Pilot 要買的是
**「第二檔資料不齊時，第一檔不會被污染」這件事在真資料上成立**——
不是多幾檔的覆蓋率。選檔刻意 heterogeneous：一檔**非美元報價**
（讓報價單位路徑走真資料，例 SIVE.ST／IQE.L）＋一檔**沒有人工分部觀測**的（驗 Q3 缺料）。

⚠ **Pilot 的每一檔都要新增三本 ledger 的判斷，那是 pq2 研究工作不是開發工作**
（判準：`go` 之後改變的是「我知道什麼」）。本檔只負責「Pilot 這個開發項的驗收條件」：
**每加一檔，既有那幾檔的 baseline 數字必須 0 格改變**，且
`tests/test_full_chain_acceptance.py` 的 35 條在新檔上同樣成立。
前置：無（Step 4 的備份條件已於 2026-09-07 解除）。⚠ 但**異地副本仍缺**（`drive: skipped`）——Pilot 會讓 private authority 的量變成三倍，在那之前補上 Drive 上傳比較划算。
