...
| **2** | First research vertical slice（**標的＝COHR**） | B4：concrete `GraphResearchProvider`、**session assessor（不接 API）**、`python -m alpha research COHR --as-of ...` | ①每個 score 都能 explain 到 `EvidenceRef`；②圖零新增節點；③與 `rank_bottlenecks` 首選一致或能說明差異 | Phase 1 |
