...
| **3**<br>🔶進行中 | Engine D decomposition ＋ `mcp_server` domain 抽出 | B2／B3／B5／B6：shared infra 升格、alpha 模組搬入、`sizing` 切三段、`brief` 拆四 pane；core → `mcp_server` import 5 → 0 | 🔶 **3a／3b 已完成（2026-09-03）**：涉及 `decision_lab` 的相依環 **3 → 1**（只剩 composition root）；**core → `mcp_server` import 5 → 0**；`mcp_server/` 4,016 → **673 行**（domain 2,261 行抽到 `intake/`）；golden fixture 漂移 **0**（兩批都是）；1,394 → **1,413 passed**。<br><br>🔶 **3c／shim 清空已完成（2026-09-04）**：`AlphaSignal → Engine D` adapter 接通研究→決策；**搬遷 shim 15 → 0**，清空當下現形兩條被它藏住的 `decision_lab → portfolio`（一條已修回 `risk.snapshot`，一條進具名的 `PENDING_B6_COUPLINGS`）；四個 infra 檔搬 `shared/`；`decision_lab/` 10,653 → **9,981**；1,435 passed；突變 **39 個空跑 0**；`audit invariants` **FAIL 0**。<br>剩餘：`decision_lab/` ≤8,000 行（現 9,981，缺口幾乎全在 `brief.py` 1,462 → ~400）、`brief` 拆 pane（B6）、daily bytes 下降 | Phase 2 |
