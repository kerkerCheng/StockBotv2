...
|---|---|---|---|
| **Agent Development Flow v1（token-aware bootstrap）** 🔶 **2026-09-07 交付，待人工驗收** | 開發請求目前沒有統一入口：使用者從「我想做什麼」開始，而 agent 得自己猜要站多遠看問題、要不要開第二個 context。實際成本落在兩處——**跨 App 複製貼上**，以及**沒有判準時預設多開一個 agent**（每次 spawn 都是冷啟動，要重新推導主 agent 已有的 context）。同時 `blind-spot-audit` 的 15 個 lens 有 3 個（B8／B9／C10）整段建立在**隨 U7 移除的資本表達層**與 **Engine D 舊責任**之上——`supported_range`、五軸 ceiling、「D 的 supported range 怎麼回到使用者」——reviewer 會對著不存在的機制開火。⚠ RSI／MACD 這組 beta 訊號字彙**原本就不在該 skill 裡**，測試對它們的禁令是預防不是修補 | ✅ 新增 `docs/AGENT_WORKFLOW.md`（Zoom Z0–Z3／Review R0–R2／七欄 STEP_RESULT／四值 verdict／七種 message／R2 六條 trigger）＋`skills/development-flow/SKILL.md`（intake、routing、development 與 operational 兩個 review profile）；`skills/blind-spot-audit/SKILL.md` 收斂成 research profile（B8／C10 RETIRE、B9 REWRITE、D13–D17 與 C12 MOVE_TO_DEVELOPMENT／OPERATIONAL、A2 REWRITE 成「Convergence without differentiated evidence is healthy」、新增 A6 假設出處／A7 thesis→model／A8 相關性與退出）。<br>**canonical Markdown 3 份（hard max 4）**，機械擋住碎檔重生。<br>驗收：1,989 → **2,001 passed / 1 skipped**；突變 180 → **186 個**（新增六個全部證明會紅，空跑 0）；`audit invariants` FAIL 0｜PASS 12｜SKIPPED 0；`sync_agent_skills.py --check` 無漂移；`.codex/rules` 的 `prefix_rule(` 仍是 16。<br>⚠ **這個 Phase 不得用自己剛建立的流程宣告自己有效**——bootstrap 由既有人工雙邊流程驗收，第一次 dogfood 的結果才是第一份證據。<br>查證：`python -m pytest tests/test_agent_workflow.py -q`／`python scripts/verify_test_nonvacuity.py --only AgentFlow`／`python scripts/sync_agent_skills.py --check` | — |
