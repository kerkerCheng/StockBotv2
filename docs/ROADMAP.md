...
| 項目 | 為什麼 | 驗收條件 | 前置 |
|---|---|---|---|
| **Engine C 行情層缺 TSM 序列——TSMC cohort 的 valuation 軸結構性歸零** | 2026-09-01 [275] 實測：registry 有 `research_ticker: TSM`，但 reassess 帶 `--ticker TSM` 仍 `market_missing`／`fx_missing`——Engine C 行情 ETL 的抓取範圍不含 TSM，frozen context 永遠沒有價格序列可引用。跨陣營製程瓶頸頂點的 cohort 因此 valuation 軸只能 unknown | TSMC cohort reassess 後 `market_missing` 消失、`yfinance://history/TSM` 出現在 reference_index。查證：`python -m decision_lab references dc_29ef9f7194dc996c9b7eb62a9b21afc6` | 先找 ETL universe 的實際來源（config 還是 cohort 導出），確認加 TSM 是一行 config 還是要動抓取邏輯 |
