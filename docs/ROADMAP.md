...
| **`execution_fx_missing`／`live_nav_missing` 未登記進 `config/decision_blockers.json`** | 兩者未命中 exact／prefix → 判 unknown → `_only_system_internal_blockers` 回 False，於是即使其他 blocker 全部乾淨，項目仍被釘在「等你決定」佇列。它們是 live-lane 的系統內部狀態（沒讀到 Sheet 就沒有 NAV、沒有執行 FX），**不是使用者能決定的事**——正是分類表自己的漏網。2026-08-17 實測：AXT／NVDA 重跑後 `assessment_blockers`／`paper_blockers` 都空了、paper 回 ELIGIBLE，[163]／[164] 仍不退場 | 兩碼登記後，只剩 live-lane blocker 的 `decision_review` 項目會走 `system_internal_only` 的 deterministic retirement audit 而不再詢問；以 [163]／[164] 為 before → after 樣本 | 先確認它們是否只是 `holdings_unavailable` 的下游連鎖——若上游修好就一起消失，這條可能自動消解，不必動分類表 |
...
| 🔴 **`financial_snapshots.price` 取自 `yfinance.info`，且 `bar_date` 是憑空生成的** | 2026-08-18 實測：COHR 記 `snapshot=08-18, bar_date=2026-08-17, kind=close, price=351.22`，但**08-17 根本不是交易日**（週五 08-14 → 週二 08-18），且 351.22 對不上任何一天收盤（08-14 收 325.83、08-18 收 309.00）。來源是 `yfinance.info.previousClose`，provider 自己就不可靠；ETL 再幫它蓋一個推導出來的 `bar_date`，於是錯的價格帶著**看起來很權威的日期**流向下游。實際造成 `repricing_check` 把 COHR 距分析師目標算成 +17%（實際 +33%）、AXTI 算成 -4.6%🔴（實際 +8.5%🟠）——**兩檔的結論都被翻轉**。`outcome_if_settled_today.py` 早已避開並在 docstring 寫明，但知識沒有繼承到新檔案 | `financial_snapshots.price` 改取自實際 K 線並讓 `bar_date` 等於**真實 bar 的日期**（非交易日不得產生列）；以 COHR 08-18 那筆為 before → after 樣本。消費端（`engine_c.checklist`、`repricing_check`）不再需要各自繞道 | 無 |
