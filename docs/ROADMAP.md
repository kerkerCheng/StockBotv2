...
| **`evidence` 三級不分「客戶端具名」與「第三方媒體提及」** | 兩者都算 `externally_corroborated`，但強度差一個量級：NVIDIA 在自家 PR 點名 COHR，與媒體報導提及，可稽核性完全不同。這使排序無法反映最硬的那類證據 | `evidence` 增加一級並讓 `EVIDENCE_RANK` 反映；驗收是至少一條邊的排名因此改變 | 先確認圖中有足夠的客戶端具名實例可區分 |
| 🔴 **chokepoint 覆蓋掃描會被建模不一致污染，「0 供應商」不等於沒研究過** | 2026-08-20 實測：`tech:robotic_actuator` 在覆蓋掃描顯示 **0 個公司供應商**，但圖中**早已有兩家的逐字證據**——Boston Dynamics 官方頁面（客戶端印證，tier 2）載明 Hyundai Mobis「will supply actuators for Atlas」，Schaeffler Q1 2026 法說載明其 rotary actuator platform「covering ~80% of market demand」。問題是既有的邊建成 `co:hyundai_mobis -[supplies_to]-> co:boston_dynamics`（公司對公司）與 `co:schaeffler -> prod:… -[is_component_of]-> tech:humanoid_robot_systems`，**沒有任何一條連到 `tech:robotic_actuator` 這個 chokepoint 節點**。同一份 extraction 甚至同時建了 `tech:robotic_actuator -[is_component_of]-> prod:atlas_humanoid_robot`，卻沒把供應商接上去。後果：以「供應商數」找研究缺口的掃描會把**已研究過的領域誤報成空白**，而這正是選題的輸入 | 掃描時對每個 0 供應商節點，同時檢查是否存在「經 `prod:` 或公司對公司邊間接連到它」的公司；至少讓 `tech:robotic_actuator` 不再顯示 0。⚠ 補邊需走 graph admission，且新 extraction 不能沿用既有 `doc_id`（`inspect_provenance` 會判 conflict）——修法要先決定是補新 RA 還是走 L10 的 graph 重建授權 | 無 |
| 🔴 **事件監控對 alpha live 部位零覆蓋，且結構上不可能觸發** | 2026-08-18 開出系統第一筆真實部位（COHR 10 股 @ US$316.23，約 0.732% NAV）。08-17 收盤 351.22 → 08-20 收盤 290.03，**三個 session −17.4%**，而系統沒有任何路徑會發現。查 `portfolio_risk.py::event_search_requests()`：它只走 `config/beta_policy.json` 的 `instruments`，且要求 `leverage_multiple == 1.0` 且 `issuer_loads[issuer] == 1.0`，已登記 issuer 只有 TSMC／ALPHABET／MICRON／NVIDIA／TESLA。**COHR 不在其中，所以跌多少都不會產生 packet**；即使在，門檻也是 20% 曝險 vs 實際 0.73%。這不是程式 bug——事件監控本來就是 Beta lane 機制——但它在第一筆 alpha live 部位開出那天起，就成了**對最需要它的 lane 完全沒有覆蓋的監控**。依 L14 第 4 點的三個免 outcome 測試，這是「**恆滅**」（清除率近 0 的反面：觸發率恆為 0），鑑別力同樣是零 | alpha live 部位的單日跌幅首次產生一筆 `event_search_requests`（或等價的 alpha 專用 packet）。⚠ 門檻**不得沿用** beta 的 20% 曝險——alpha 單筆上限本來就是 5%，沿用等於再造一個恆滅閘門；門檻應 keyed 在「該部位是否有 live fill」而非曝險占比 | 無。但需先決定它掛在 Engine D（有 `live_execution_reports` 才是真部位）還是 beta_monitor |
