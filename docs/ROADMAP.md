...
| **ETF 完整 look-through** | `issuer_loads` 只涵蓋 policy 已登記的 ownership，曝險輸出恆為 `partial`。⚠ 2026-09-02 評估：這不是 config 補幾行的事——`coverage: full` 需要**該 ETF 的完整成分權重資料源＋鮮度機制**（靜態快照會悄悄過期，比誠實的 partial 更危險）。可行源：QQQ＝Invesco 官網每日 CSV（免費）；台股 0050/00631L＝發行人 PCF（無穩定 API）。是一個「建 fetcher＋staleness 現形」的完整開發項，不是清尾巴 | 曝險輸出出現 `coverage: full` 的標的 ≥1，且成分資料帶 as-of 日期與 stale 現形 | 先選定成分資料源（建議 QQQ 的 Invesco CSV 起步）再建 fetcher | 選資料源 |
