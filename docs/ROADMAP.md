...
| **5 量測** ○ | 量測層從 trade_log 加收據重建（舊 Decision Store 的 outcome 只當歷史）；主題等權籃子**沿用 Phase 3 定義的主題等權組**（2026-09-26 amendment A2 移入 Phase 3）；追蹤表加籃子基準與三個 power-law 統計量；圖預測對錯表（讀圖斷言 vs 後續證據）；帳號計分表接籃子超額 | 報酬是慢迴路，圖的預測是快迴路（G9） | ①追蹤表印籃子超額；②圖預測表有第一筆對／錯；③計分表印量測起始日與樣本數 | 3 |
