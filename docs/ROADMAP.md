...
| **4a** ✅<br>**4b** ▶ | 用機械條件挑出倍率候選，挑不出就誠實說空 | 4a：filter 逐檔報 input／accepted／filtered／reasons ✅<br>4b：~~每一格配 disproof~~ **2026-09-20 修訂：有賭注的檔，其賭注所依賴的 driver 有反證在盯** | 4a ✅（三條已接、兩條量完確認不接）<br>4b：**3 檔有賭注、0 檔連得上反證**。[638] 會讓第一檔（COHR）通過；另兩檔是 AXTI／LITE。查證：`python scripts/multi_year_check.py` |
...
- **多年橋不產生價格**，它產生的是「要 2 倍，資料中心分部得四年累積成長 4.65 倍」
