...
| **✅已結案 2026-09-02（量測後留著不動）：等待機制三套在 Event Watch 之外** | 前置量測完成，三套「看起來在等、實際不會醒」實例**全為 0**：①RA expiry——99 個 RA 中 2 筆正確轉 `expired`（檢查者實證有效）、5 筆 `partial` 均有 `expires_at` 兜底；②thesis lifecycle——active epoch 1 個、`review_due` 逾期未動 0；③catalyst calendar——27 檔 0 overdue，6 筆設定不完整**自我現形**（表內明標「到期提醒是假的」）。對照 trace 併入時的答案是「10 筆假死變 0」——這三套答不出對應數字，依 L14 留著不動。catalyst calendar 另有 L7 的 48 小時動作語意，獨立是正確設計。查證：`python scripts/catalyst_watch.py` 首行；RA state 分布見 `library/private/research_actions/*.json` 的 `state` 欄 | 三套的到期／觸發改由 registry 判定，或**明確記錄為何該獨立** | ✅量測完成（見左） |
...
| **✅已交付 2026-09-02：`structural_lead_time_weeks` 語意限定為 qualification＋既有誤標改正** | 實測時發現全圖真值已有**兩筆**：GF 52 週（quote 明寫「lead times for **delivery**」＝交貨週期，誤標）與 force_torque_sensor 156 週（「3-7 years from program start to production-qualified device」＝**正確的 qualification 語意**，且該 claim 自己就示範了「低替代難度＋長合格週期」的區分）。處置：①抽取指引（`prompts/extract_system.md`）限定 structural＝qualification only、新增 `delivery_lead_time_weeks` 承接交貨週期並附誤標事例；②GF 那筆於圖（assertion attributes）與 tracked 抽取檔同步改標為 delivery，帶 relabel 稽核欄；③`bottleneck.py` 已知限制改寫：qualification 常年近空是誠實狀態不是待填欄。改後 structural 真值＝1（僅正確語意那筆）。查證：`grep -l structural_lead_time extractions/*.json`＋圖上 `e.lead_time_relabeled_at` | 欄位拆成 delivery 與 qualification 兩個，或明確限定只填 qualification 並把既有那筆改標；抽取指引說明差異 | ✅（採「限定＋拆欄」並行） |
| **✅已交付（2026-09-02 查證確認；實作於先前 loop）：`evidence` 五級分級** | 修正後階梯已全在 `query/bottleneck.py::EVIDENCE_RANK`：`externally_corroborated`(4)＞`counterparty_joint`(3)＞`self_reported_costly`(2)＞`needs_review`(1)＞`self_reported`(0)；costly 用「出自 filing（審計／法律責任文件）」當確定性 proxy（註解明講 proxy 限制），joint 用「origin 字串具名 ≥2 家 registry 公司」偵測。**驗收達成**：2026-09-02 實測可行動排序 34 條的分布為 外部印證 9／costly 11／待判定 6／自報 8——11 條邊因 costly 級排上敘述性自報之前 | 排序無法反映最硬的那類證據 | `EVIDENCE_RANK` 反映新級；至少一條邊排名改變 | ✅ |
...
| **✅已結案 2026-09-02（使用者定案：不建管線，LLM 當下概算即可）：ETF 完整 look-through** | 原方案要建成分資料 fetcher＋鮮度機制才有 `coverage: full`；使用者定案「給 LLM 當下看就可以，大概就好」——需要更完整的曝險圖像時，由互動 session 當場查該 ETF 現行成分（發行人官網/公開來源）做**概算**，明標「概算·未經查證·as-of 當日」，只出現在對話，**不寫進 `issuer_loads`**（deterministic 的 partial 契約不變，「已知至少 X%」照舊；L15：LLM 可解析與提議，不可成為 authority）。這正好避開靜態快照悄悄過期的風險——概算每次現查，天然新鮮 | `issuer_loads` 恆 partial | 使用者要看時能得到帶標註的概算 | ✅ |
...
| **🔶併入大項 B（2026-09-02）：Token-efficient Daily Runner 重構** | 與 B 的驗收①是同一件事（單次 daily 輸出/token 下降且 pq2 項目數不減），分兩項會重複量測。baseline 已由 notifications 留底解決（2026-09-02＝24,195 bytes）；後續進展記在 B，不另立 | daily 的 token 成本 | 併入 B 的驗收① | 見 B |
| **✅已交付 2026-09-02：Workstream B Paywall ROI 清單** | 盤點至今實際遇到的 paywall（11 筆命中去重成 3 類）：①賣方研究——機構通路個人買不到，且唯二實例一個已 L8 隔離、一個使用者已 drop；②法說 transcript——唯一重複真缺口（COHR/MTSI 各一句關鍵 quote），Seeking Alpha US$299/年 vs 既有零成本 `transcribe_audio.py` 路徑，**建議不訂閱**（頻率兩個月 2 例）；③誤命中 2 筆記錄防重查。**結論：目前無任何值得核可的付費項**。報告：[`docs/reports/paywall_roi_2026-09-02.md`](reports/paywall_roi_2026-09-02.md) | 付費來源何時值得買、合法人工取得路徑 | 產出清單使用者可逐項核可 | ✅ |
| **✅已結案 2026-09-02（前提過期——窄版早於 2026-08-01 交付）：Sheet writer** | 本項前提「現行所有 runtime 都不寫 Google Sheet」**是假的**：`scripts/record_trade.py`（2026-08-01 使用者定案）就是窄版 Sheet writer——記手動成交時 append `trade_log.jsonl` 事件流＋寫 Sheet 持股儲存格（僅股數/現金/均價；欄名＋內容定位、寫前重讀確認、預設 dry-run、逐儲存格絕不寫範圍），2026-09-01 VWRA 170 股實跑留有 `sheet_writes` from/to 收據。AGENTS「日常 credential 只有 readonly」管的是無人值守排程，與逐筆手動觸發的 record_trade 不衝突。**更廣泛的寫入從未有具體需求**，使用者 2026-09-02 已想不起本項所指——結案；未來有具體要寫的欄位再立新項。⚠ 本次結案同時是一次「引用自家文件現況陳述前未跑查證」的 L11 實例：agent 先照抄本行對使用者說「都不寫 Sheet」，被使用者以 VWRA 記憶否證 | 原文：需求尚不具體 | 需求具體化才動工 | ✅（窄版已存在） |
| **✅已結案 2026-09-02（重評後不做）：Confidence 五軸重構為三類** | 前置的重評完成。三類設計（否決／信心／賠率）誕生於五軸還在**決定資本上限**的時代；2026-08-28 U7 已把資本表達層整組移除，五軸現職是「排序＋指出最弱軸」——「賠率類」原本要解的問題（尺寸該隨賠率放大）**已無載體**：系統不給尺寸，賠率判斷由使用者在 variant perception（市場隱含 X vs 本 thesis Y）與 `valuation_payoff` 軸表達。剩餘的「如何量化賠率」在無尺寸系統裡是研究題不是機制題。若未來恢復任何資本表達（需先過 L14 量測），屆時再重開 | brainstorm §4 | §4 第 4 步已完成；重評結論如左 | ✅重評完成 |
...
| **✅已結案 2026-09-02（量測後不實作）：待辦池沒有 evidence conflict 類型** | 前置量測完成：08-19 至 09-02 兩週為本圖史上最重度的 drain 期（本週 22 個 RA 入圖、多次 decompose），`open_conflicts` 仍為 **0**（raw derived 22 全有 resolution，與 08-19 相同）——conflict 產生率實測 ≈ 0/兩週，滯留 0。依本項自己的判準「有非零滯留才實作；否則是替不存在的問題加機制」，不建 pq2 類型。若未來 drain 後發現 open conflict，處置仍是入圖當輪順手 resolve（0→22 全是這樣清的）。查證：`python loader/edge_resolution.py project --dry-run` 讀 `open_conflicts` | 先累積幾輪 drain 的 conflict 產生率與平均滯留時間，有非零滯留才實作 | ✅量測完成（見左） |
