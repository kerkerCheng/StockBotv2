...
| **3c** | `skills/system-decompose/SKILL.md`——使用者可隨時呼叫，週掃只執行積壓題＋提名候選（**不可讓無人值守排程自己選題**，它只能從圖裡挑＝原地繞回） | 首次拆解**未知層 Z ≥ 1**，且最終長出 ≥1 條有供應商的邊。⚠ Z=0 的正確結論是「拆得太粗」，不是「覆蓋完整」 | 🟡 SKILL 已建（2026-08-22），**尚未實跑**——驗收數字 `Z` 仍未產生，未達完成 |
