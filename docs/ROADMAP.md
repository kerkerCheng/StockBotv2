...
| **4** ○ | 篩選 | **D11 篩選層機械條件**（覆蓋家數上限、市值上限、瓶頸業務占營收下限、至少一條外部印證的瓶頸邊、入圖前 30 天漲幅上限、12 個月內指名假設的催化劑）＋籃子首選換成這組條件 | 首選是 filter 不是分數；今天的三條是賭注條件，不是倍率候選的條件；需求側節點與供給側候選今天被當成同一種東西（舊 backlog 2026-09-11 列） | filter 報表印 input／accepted／filtered 與 reasons 計數（INV-3）；`top_pick` 的判定條件由三條變六條；純需求側節點從候選集合裡被明確 filtered 而不是消失<br>**2026-09-17 併入（使用者定案，Phase 1 §2e 第 4 項）：六個 tech 節點走不到任何需求錨**——`tech:photodiode`／`tech:pluggable_transceiver`／`tech:transceiver_1_6t`／`tech:cloud_transceiver`／`tech:inp_dfb_laser`／`tech:cpo_full_stack_test`（實測 shortestPath 回空）。**這是圖的結構缺口，不是公司問題**：補在這些節點下的供應商邊入圖後仍然排序不可見。併這裡的理由是它正是本 Phase 要分的那件事——需求側節點 vs 供給側候選今天被當成同一種東西。驗收多一條：**這六個節點必須在 filter 報表裡以明確 reason 出現，不得只是消失**。查證：`python -m query.bottleneck --top-n 60`（末列的🔴走不到錨提示）| Phase 1 |
