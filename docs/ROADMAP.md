...
|---|---|---|---|---|
| **Graph MCP 退役** ○（2026-09-24 使用者決定；plan 2026-09-24-001 §0.1 C5） | 拆 `mcp_server/`（`graph_mcp.py`、`leads_tools.py`、`engine_c_tools.py`）與其測試；`docs/remote-access-architecture.md` 封存進 `docs/archive/`、OPERATIONS「MCP server」節與 ARCHITECTURE 的 MCP 段改寫；`intake/` 的 domain 留下（本機路徑仍在用）；`scripts/retired_mechanism_grep.py` 加一組 MCP regex；AGENTS「cloud＋MCP 是備援」一句的改寫另給五欄 amendment 請使用者核准（判準句） | 手機／網頁 connector 不再使用；它是一個對外的寫入入口（`record_lead_decision`、`apply_research_action`、`load_extraction`），不用就不該開著。已先停：2026-09-24 process 停、開機 vbs 移除；tunnel 的 `mcp.`、`neo4j.` hostname 由使用者移除 | ①`mcp_server/` 不存在；②MCP regex 的殭屍 grep 扣 keep-list 為 0；③`pytest` 綠、`python -m audit invariants` 綠；④`~/.cloudflared/config.yml` 沒有 `mcp.`、`neo4j.`（查證：`grep hostname ~/.cloudflared/config.yml`）；⑤AGENTS 那一句的改寫有使用者核准紀錄 | 無（Phase 1 結案後做，避免同時兩個 writer 改同一批檔） |
