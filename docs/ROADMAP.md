...
| **1 等待與心跳** ▶（plan [`2026-09-24-001`](plans/2026-09-24-001-feat-phase1-waiting-heartbeat-plan.md)；2026-09-24 使用者定案 amendment A1–A4，五欄見 plan §0.4） | **排程（A1）**：一個 Windows daily（零 LLM、封閉步驟清單：抓資料、ETL、FX、watch 比對與到期處置、sync、materialize、健康審查、invariants、本機備份、心跳、每天唯一一則 Discord），取代 Heartbeat／FxSync 兩個工作；時間只住 `config/daily_routine.json`，註冊命令由它導出、每次執行自我比對；Codex 只做 triage；長版 Daily Brief 退出無人值守；weekly 退役（題材掃描改互動 skill，Discord 與 session 開頭常駐「距上次掃題材 N 天」）。**等待**：Event Watch 加 `semantic_condition` kind（條件原文、實體、層或插槽、來源指標、核查頻率、48 小時動作、到期）；讀圖（contract v2 `disproof[]`）與 thesis（envelope 結構化反證）寫反證時自動登記，敘事的登記移到 Phase 3（A2）；411 條舊反證只登記被讀圖或 thesis 引用的——既有 16 條 thesis 反證與 2 份現行讀圖的反證由強模型補登記；語意 watch 只比對一手文件（排除持股申報）、不等 triage，醒來＝待檢，判定在互動 session，Codex 預篩額度旋鈕預設 0（A4）；`entity_filing_signal` 加 wake target `wake_reading`（客戶節點出新文件 → 該層讀圖標 stale）；到期（A3）：語意／假設型鑄 pq2 新 kind `watch_decision`（`GO_AUTHORIZATION`：go＝現在啟動研究、不含任何 authority；續等＝watch 以新到期日回 active、等待只住 registry；`drop`＝放棄），pq2 型翻回「球在你」，追源型 lead 轉終局並計數。**心跳**：較昨 diff（昨天快照落地）＋ watch 今日醒／到期／標旗／未檢 N ＋ 反證在盯／未盯 ＋ pq2 逐筆（go 授權／不含）＋ harvest 沒跑必亮 ＋ 分類層上次跑 ＋ 備份、健康審查、NAV、計分表每天 | 出場只認反證，反證沒人盯之前其他都是空的；registry 已存在且沒漏，缺的只是語意條件（決定紀錄 §1.5）；2026-09-24 實測 Codex 暫停讓抓資料與喚醒全停、心跳卻照印「未 triage 0」——「LLM 失敗心跳照發」必須在結構上成立 | ①watch 中 `semantic_condition` 筆數 0 → ≥ 16（3 份 thesis 的反證條目）＋ 2 份現行讀圖的反證條數（A2；Step 1.6 逐字列出後定數）；②心跳出現第一次真實的「今日醒 ≥1」；③AI 預篩關閉時心跳印「未檢 N」且 N>0；④到期的語意／假設 watch 出現在 pq2 為 `watch_decision`（`user_decision`），不消失；②③④ 結案時若尚未自然發生：測試證明機制、照實寫「已交付、未生效」並登記回查 date watch（④ 最早 2026-11-22 才可能，A3）；⑤一個 Windows daily、每天一則 Discord、排程時間與 config 一致（機制存在與否，A1） | 0a |
