...
| ✅**（2026-08-31 已交付）waiting_on trigger 可達性＋Event Watch 模組**——F1/F2/F3 全落地（`engine_b/event_watch.py`：T0/T1 sync 喚醒＋T2 sweep K 旋鈕＋計數器；驗收 baseline 0/3→3/3 達成）。原稽核背景見 [`docs/reports/trigger_audit_2026-08-31.md`](reports/trigger_audit_2026-08-31.md)） | 使用者問「等事件的量大了之後真的喚得醒嗎」。⚠ 2026-08-31 稽核**下修昨日結論**：[81] 並非「until 機器可達」——**全 codebase 沒有任何程式比對 `until` 與今天**（[81] 實際騎在 RA `expires_at` 上僥倖可達）；3 個 waiting 項機器可達其實是 0。對照組：trace 端（`related_entity_signal`／`primary_source_signal`＋consumed-marker＋死件自標）是成熟引擎，49 筆等待全可達——**兩套等待、一套引擎閒置**：[200] 等的 Agility S-4 落地成 EDGAR lead 時會喚醒相關 trace，卻不會喚醒 pq2 [200] | ①F1：sync 比對 `until`，到期翻回「等你決定」＋`until_expired` 標記（不自動 drop/go）；②F2：`event_link` 新增 `entity_filing_signal {entities}` 複用 trace 引擎——tier-1 新 lead 共用具名標的即喚醒 pq2 項回 user_decision；與截圖 brainstorm 的 `fact_verification` 共用底座；③F3：daily 首屏常駐「等事件 N（機器可達 M）／trace backlog K（死件 J）」。baseline：waiting 3 項可達 0 | 無 |
