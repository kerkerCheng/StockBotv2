...
**Step 5 ✅ 2026-09-07 完成（CONDITIONAL GO）**，報告見
...
**⚠ CONDITIONAL 的條件（未完成，需使用者本人操作）：** Cloudflare 端三步——
①Zero Trust 建 Access application（One-time PIN 即可，不需 OAuth）
②`cloudflared tunnel route dns d3074ec2-… stockbot.minatoyukina.uk`
③加一條 ingress 並重啟 cloudflared。
**步驟 1 必須排在 2 之前**——DNS 一建立 hostname 就開始解析，那時若還沒有 Access policy，
這個網址是**公開可讀**的。程序、驗收指令與安全邊界見
